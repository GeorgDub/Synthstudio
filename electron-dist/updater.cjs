...
async function loadAutoUpdater() {
  try {
    const updaterModule = await Promise.resolve().then(() => __toESM(require_main2(), 1));
...
async function setupAutoUpdater(mainWindow, onLegitimateQuit) {
  ...
  autoUpdater.on("update-available", (info) => {
    ...
    console.log(`[Updater] Update verf\xFCgbar: v${info.version}`);
    import_electron.dialog.showMessageBox(mainWindow, {
      type: "info",
      title: "Update verf\xFCgbar",
      message: `Version ${info.version} ist verf\xFCgbar.`,
      detail: "Soll das Update jetzt heruntergeladen werden?",
      buttons: ["Herunterladen", "Sp\xE4ter"],
      defaultId: 0,
      cancelId: 1
    }).then(({ response }) => {
      if (response === 0) {
  ...
  autoUpdater.on("update-downloaded", (info) => {
    ...
    import_electron.dialog.showMessageBox(mainWindow, {
      ...
      buttons: ["Jetzt neu starten", "Sp\xE4ter"],
      defaultId: 0,
      cancelId: 1
    }).then(({ response }) => {
      if (response === 0) {
...
async function checkForUpdatesManually(mainWindow) {
  ...
  if (!available || !autoUpdater) {
    import_electron.dialog.showMessageBox(mainWindow, {
      type: "info",
      title: "Auto-Updater",
      message: "Auto-Updater nicht verf\xFCgbar",
