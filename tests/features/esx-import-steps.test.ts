...
import type { EsxBank, EsxPattern, EsxPart, EsxStepEvent } from "../../client/src/utils/korg/esxParser";

function step(active: boolean, velocity = 100): EsxStepEvent {
  return { active, velocity };
}

function makePart(partIndex: number, activeSteps: number[], opts: Partial<EsxPart> = {}): EsxPart {
...
function makePattern(index: number, name: string, parts: EsxPart[]): EsxPattern {
...
function makeBank(patterns: EsxPattern[], opts: Partial<EsxBank> = {}): EsxBank {
  return {
    source: "TEST.esx",
...
describe("esxBankToImportResult", () => {
  it("überträgt echte Step-Trigger (nicht leere Templates)", () => {
    ...
    const activeIdx = kick.steps.map((s, i) => (s.active ? i : -1)).filter((i) => i >= 0);
    ...
    const snareIdx = snare.steps.map((s, i) => (s.active ? i : -1)).filter((i) => i >= 0);
  ...
  it("warnt über fehlendes Sample-Audio wenn Samples in der Bank sind", () => {
    const bank = makeBank(
      [makePattern(0, "P1", [makePart(0, [0])])],
      { monoSamples: [{} as never, {} as never], stereoSamples: [{} as never] },
    );
    const result = esxBankToImportResult(bank, "TEST.esx");
    expect(result.warnings.some((w) => /3 Sample/.test(w))).toBe(true);
    expect(result.warnings.some((w) => /KORG-Bank/.test(w))).toBe(true);
  });
