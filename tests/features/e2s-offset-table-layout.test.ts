...
describe("E2S .all offset-table geometry", () => {
  it("starts at 0x0010 with 1020 entries (Oe2sSLE)", () => {
    // Absichtlich literal: ein symbolischer Test kann einen falschen Wert
    // nicht bemerken, weil er ihn selbst benutzt.
    // SoT: Oe2sSLE `e2s_sample_all.py` load() liest
    // struct.unpack("<"+"I"*1020, f.read(4080)) ab 0x0010.
  ...
  it("fills the prelude exactly — table end == sample-area start", () => {
    ...
    // Die Rechnung, die den ersten Fehlwert entlarvt hätte:
    // 0x07E0 + 250*4 = 0x0BC8, und das ist nicht 0x1000.
    expect(0x07e0 + 250 * 4).not.toBe(E2S_ALL_SAMPLE_AREA_START);
    // ⚠️ Achtung, kein Diskriminator: die Zwischenstufe 0x0058/1002 füllt das
    // Fenster EBENFALLS exakt aus (0x0058 + 4008 = 0x1000). Die Fenster-
    // Arithmetik allein konnte 0x0058 daher nicht widerlegen — erst der
    // Abgleich mit Oe2sSLE tat es.
    expect(0x0058 + 1002 * 4).toBe(E2S_ALL_SAMPLE_AREA_START);
  });

  it("begins immediately after the 16-byte signature — no header rest", () => {
    // Unter der Oe2sSLE-Geometrie gibt es keinen reservierten Header-Rest
    // mehr: die Tabelle schließt direkt an die Signatur an.
    expect(E2S_ALL_SIGNATURE_LEN).toBe(16);
    expect(E2S_ALL_OFFSET_TABLE_START).toBe(E2S_ALL_SIGNATURE_LEN);
  });

  it("explains the old 0x07E0 reading: it was index 500 of this table", () => {
    // Die Referenzdatei, aus der 0x07E0 abgeleitet wurde, hatte ihr erstes
    // Sample auf OSC_0index 500: 0x0010 + 500*4 == 0x07E0. Daher sah die
    // Tabelle aus, als beginne sie dort.
    expect(E2S_ALL_OFFSET_TABLE_START + 500 * 4).toBe(0x07e0);
  });

  it("exposes 1000 user-pickable slots within a 1020-entry table", () => {
    // E2S_SLOT_INDEX_MAX ist ein GERÄTE-Limit (Sample-Nummern 0..999) und
    // unabhängig von der Tabellenkapazität. Die frühere Beziehung
    // "MAX_SLOTS - SLOT_INDEX_MAX === 2" war ein Artefakt der 1002-Lesart.
    expect(E2S_SLOT_INDEX_MAX).toBe(1000);
    expect(E2S_MAX_SLOTS).toBeGreaterThan(E2S_SLOT_INDEX_MAX);
  });
});
