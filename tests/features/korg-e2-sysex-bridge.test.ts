...
function makeBody(name: string, oscRefs: Record<number, number> = {}): Uint8Array {
  const body = new Uint8Array(0x4000);
  for (let i = 0; i < name.length; i++) body[PATTERN_NAME_OFFSET + i] = name.charCodeAt(i);
  for (const [pStr, ref] of Object.entries(oscRefs)) {
    const off = PART_TABLE_OFFSET + Number(pStr) * PART_STRIDE + PART_OSC_REF_OFFSET;
    body[off] = ref & 0xff;
...
const ACK = Uint8Array.from([0xf0, 0x42, 0x30, 0x00, 0x01, E2Model.SAMPLER, E2Func.ACK, 0xf7]);
const NAK = Uint8Array.from([0xf0, 0x42, 0x30, 0x00, 0x01, E2Model.SAMPLER, E2Func.NAK, 0xf7]);
function identityReply(model = E2Model.SAMPLER, ch = 0, vMaj = 2, vMin = 2): Uint8Array {
  return Uint8Array.from([0xf0, 0x42, 0x50, 0x01, ch, 0x00, model, 0, 0, 0, vMaj, vMin, 0xf7]);
}
...
function fakeDevice(responder: (frame: Uint8Array) => Uint8Array[] | null): {
  ...
  const transport: E2Transport = {
    ...
    send(frame: Uint8Array) {
      ...
      if (replies) for (const r of replies) queueMicrotask(() => transport.onmessage?.(r));
    },
  };
...
describe("E2SysexBridge — identify", () => {
  it("sends the search request and resolves identity", async () => {
    ...
    expect(id).toEqual({ globalChannel: 3, model: E2Model.SAMPLER, versionMajor: 2, versionMinor: 2 });
...
describe("E2SysexBridge — pull", () => {
  it("pulls the current pattern and decodes the body bit-exact", async () => {
    ...
    const { transport } = fakeDevice((f) =>
      funcOf(f) === E2Func.CURRENT_PATTERN_DUMP_REQ ? [buildCurrentPatternDump(body)] : null,
    );
    const bridge = new E2SysexBridge();
    bridge.attach(transport);
  ...
  it("pulls a numbered pattern and returns a summary", async () => {
    ...
    const { transport } = fakeDevice((f) =>
      funcOf(f) === E2Func.PATTERN_DUMP_REQ ? [buildPatternDump(199, body)] : null,
    );
    const bridge = new E2SysexBridge();
    bridge.attach(transport);
  ...
  it("ignores a mismatched pattern number and times out (correlation)", async () => {
    ...
    const { transport } = fakeDevice(() => [buildPatternDump(10, makeBody("X"))]);
    const bridge = new E2SysexBridge({ timeoutMs: 20 });
    bridge.attach(transport);
...
describe("E2SysexBridge — push", () => {
  it("pushes current pattern and resolves on ACK", async () => {
    ...
    const bridge = new E2SysexBridge();
    bridge.attach(transport);
    await expect(bridge.pushCurrentPattern(makeBody("NEW"))).resolves.toBeUndefined();
  ...
  it("rejects on NAK", async () => {
    ...
    const bridge = new E2SysexBridge();
    bridge.attach(transport);
    await expect(bridge.pushPattern(5, makeBody("NO"))).rejects.toThrow(/rejected pattern 5/);
  });
  ...
  it("blocks writes while the sequencer runs (guard, edge case)", async () => {
    ...
    bridge.attach(transport);
    await expect(bridge.pushCurrentPattern(makeBody("X"))).rejects.toBeInstanceOf(
      E2SequencerRunningError,
    );
  ...
  it("honours maxChunkBytes for small-buffer transports", async () => {
    ...
    const bridge = new E2SysexBridge({ maxChunkBytes: 64, isPlaying: () => false });
    bridge.attach(transport);
    await bridge.pushGlobal(Uint8Array.from({ length: 300 }, (_, i) => i & 0x7f));
...
describe("E2SysexBridge — lifecycle", () => {
  ...
  it("uses the configured model + global channel in request headers", async () => {
    const { transport, sent } = fakeDevice((f) =>
      funcOf(f) === E2Func.GLOBAL_DUMP_REQ ? [buildGlobalDump(Uint8Array.from([1, 2, 3]))] : null,
    );
    const bridge = new E2SysexBridge({ model: E2Model.SYNTH, globalChannel: 7 });
    bridge.attach(transport);
