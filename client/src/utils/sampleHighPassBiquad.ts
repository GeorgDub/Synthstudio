/**
 * Synthstudio – sampleHighPassBiquad.ts (v3.232.0)
 *
 * Pure-Helper für RBJ-Biquad-Highpass-Filter (-12 dB/Oct, zwei-pol).
 *
 ...
 *
 * Steiler als die v3.199 1-pole-Variante (-6 dB/Oct) und mit echtem
 * Resonance-Peak über Q. Per-Channel mit frischem State; kein Cross-Channel-
 * Coupling.
 *
 * Koexistiert mit sampleHighPass.ts (1-pole) — gleiche AudioBufferLike-API,
 ...
 * - cutoffHz ≥ Nyquist         → Nyquist/2 (numerische Stabilität)
 * - q ≤ 0 / NaN / undef         → 0.707 (Butterworth)
 ...
 * ─── Tests ──────────────────────────────────────────────────────────────────
 *
 * Siehe tests/features/sample-highpass-biquad.test.ts.
 */
...
export interface HighPassBiquadOptions {
  /** Cutoff-Frequenz in Hz. Default 200. */
  cutoffHz?: number;
...
/**
 * Wendet einen RBJ-Biquad-Highpass (-12 dB/Oct) auf den Buffer an. Gibt einen
 ...
 *   - cutoffHz sehr klein       → near-identity (passt fast alles durch)
 *   - q = 0.707                 → Butterworth (max-flat im Pass-Band)
 *   - q hoch                    → ausgeprägter Peak bei cutoff (Resonanz)
 *   - DC-Input                  → wird zu 0 (HP-Gain @ DC = 0)
 */
export function applyHighPassBiquad(
  buffer: AudioBufferLike,
  options: HighPassBiquadOptions = {},
): AudioBufferLike {
  ...
  // RBJ Biquad Highpass Coefficients
...
/**
 * Curated Presets fuer typische Biquad-HP-Anwendungen.
 */
export const HIGHPASS_BIQUAD_PRESETS = {
  rumble: { cutoffHz: 80, q: 0.707 },
  vocal: { cutoffHz: 100, q: 0.707 },
  thin: { cutoffHz: 300, q: 0.707 },
  resonant: { cutoffHz: 250, q: 4 },
} as const;
