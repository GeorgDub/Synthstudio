...
export interface DrumMachineActions {
   ...
   */
  applyImportedSamples: (
    patternIds: string[],
    samplesByName: Record<string, string>
  ) => void;
  ...
  setPatternFollowAction: (
    id: string,
    action: import("../audio/AudioEngine").FollowAction | undefined
  ) => void;
  ...
  setPartGranularParams: (
    partId: string,
    params: Partial<import("../audio/GranularEngine").GranularParams>
  ) => void;
  ...
  setStepParamLock: (
    partId: string,
    stepIndex: number,
    lock: import("../audio/AudioEngine").StepParamLock | undefined
  ) => void;
  setStepLength: (partId: string, stepIndex: number, length: number) => void;
  setStepChainNext: (
    partId: string,
    stepIndex: number,
    chain: "up" | "down" | "none" | undefined
  ) => void;
  quantizePartSteps: (
    partId: string,
    grid: import("../utils/quantizeGrid").QuantizeGrid,
    strength: number
  ) => void;
  setPartEuclidean: (
    partId: string,
  ...
  applyStepCountToAllPatterns: (count: StepCount) => void;
...
function clonePatternForInsert(
  pattern: PatternData,
  name: string,
  id = makeId()
): PatternData {
...
export function applyPartColorUpdate(
  ...
  return patterns.map(p => ({
    ...p,
    parts: p.parts.map(pt =>
      pt.id === partId ? { ...pt, color: normalized } : pt
    ),
  }));
}
...
function makePattern(name: string, stepCount: StepCount = 16): PatternData {
  return {
    id: makeId(),
    name,
    stepCount,
...
export function useDrumMachineStore(): DrumMachineState & DrumMachineActions {
  ...
  const addPattern = useCallback((name?: string) => {
    setState(prev => {
      const stepCount =
        prev.patterns.find(p => p.id === prev.activePatternId)?.stepCount ?? 16;
      const p = makePattern(
        name ?? `Pattern ${prev.patterns.length + 1}`,
        stepCount
      );
      return {
        ...prev,
  ...
  const applyImportedSamples = useCallback(
    (patternIds: string[], samplesByName: Record<string, string>) => {
      if (patternIds.length === 0 || Object.keys(samplesByName).length === 0)
        return;
      ...
      updatePatterns(ps =>
        ps.map(p => {
          ...
          return {
            ...p,
            parts: p.parts.map(pt => {
              const url = pt.sampleName
                ? samplesByName[pt.sampleName]
                : undefined;
  ...
  const removePattern = useCallback((id: string) => {
    setState(prev => {
      ...
      return {
        ...
        patterns: next,
        activePatternId:
          prev.activePatternId === id ? next[0].id : prev.activePatternId,
        liveEditSourcePatternId:
          prev.liveEditSourcePatternId === id
            ? null
            : prev.liveEditSourcePatternId,
      };
    });
  }, []);
  ...
  const setActivePattern = useCallback((id: string) => {
    setState(prev => {
      ...
      if (prev.liveEditSourcePatternId && id === prev.playbackPatternId)
        return prev;
      return {
        ...prev,
        activePatternId: id,
  ...
  const copySamplesFromPattern = useCallback(
    (sourcePatternId: string, targetPatternId: string) => {
  ...
  const commitLivePatternEdit = useCallback(() => {
    setState(prev => {
      ...
      const nextPatterns = prev.patterns
        ...
        .map(p =>
          p.id === draftId && source // Draft umbenennen → Original-Name
            ? { ...p, name: source.name }
            : p
        );
      return {
  ...
  const scheduleCommit = useCallback(() => {
    setState(prev =>
      prev.liveEditSourcePatternId ? { ...prev, commitPending: true } : prev
    );
  }, []);
  ...
  const cancelLivePatternEdit = useCallback(() => {
    setState(prev => {
      ...
      return {
        ...
        patterns: nextPatterns,
        activePatternId:
          source?.id ?? nextPatterns[0]?.id ?? prev.activePatternId,
        activePartId:
          source?.parts[0]?.id ?? nextPatterns[0]?.parts[0]?.id ?? null,
        playbackPatternId: null,
        liveEditSourcePatternId: null,
        commitPending: false,
      };
  ...
  const setPatternBpm = useCallback(
    (id: string, bpm: number | null) => {
      updatePatterns(
        ps => ps.map(p => (p.id === id ? { ...p, bpm } : p)),
        false
      );
    },
    [updatePatterns]
  ...
  const setPatternBpmRatio = useCallback(
    (id: string, ratio: number | null) => {
      updatePatterns(
        ps => ps.map(p => (p.id === id ? { ...p, bpmRatio: ratio } : p)),
        false
      );
    },
    [updatePatterns]
  ...
  const setPatternBpmTransitionBars = useCallback(
    (id: string, bars: number) => {
      updatePatterns(
        ps =>
          ps.map(p => (p.id === id ? { ...p, bpmTransitionBars: bars } : p)),
        false
      );
    },
    [updatePatterns]
  ...
  const setPatternFollowAction = useCallback(
    (
      id: string,
      action: import("../audio/AudioEngine").FollowAction | undefined
    ) => {
      updatePatterns(
        ps => ps.map(p => (p.id === id ? { ...p, followAction: action } : p)),
        false
      );
    },
    [updatePatterns]
  ...
  const setPatternStepResolution = useCallback(
    (id: string, res: StepResolution) => {
      updatePatterns(
        ps => ps.map(p => (p.id === id ? { ...p, stepResolution: res } : p)),
        false
      );
    },
    [updatePatterns]
  ...
  const addPart = useCallback(
    (name?: string) => {
      setState(prev => {
        ...
        const part = makePart(
          name ?? `Kanal ${pattern.parts.length + 1}`,
          pattern.stepCount
        );
        return {
          ...prev,
    ...
    (id: string) => {
      updatePatterns(ps =>
        ps.map(p => ({
          ...p,
          parts:
            p.parts.length > 1 ? p.parts.filter(pt => pt.id !== id) : p.parts,
        }))
      );
    },
    [updatePatterns]
  ...
  const setPartSample = useCallback(
    (partId: string, sampleUrl: string, sampleName?: string) => {
      updatePatterns(ps =>
        ps.map(p => ({
          ...p,
          parts: p.parts.map(pt =>
  ...
  const setPartMuted = useCallback(
    (partId: string, muted: boolean) => {
      updatePatterns(
        ps =>
          ps.map(p => ({
            ...p,
            parts: p.parts.map(pt =>
              pt.id === partId ? { ...pt, muted } : pt
            ),
          })),
        false
      );
  ...
  const setPartSoloed = useCallback(
    (partId: string, soloed: boolean, exclusive = true) => {
      // v2.50: pure transform applySoloUpdate (testbar ohne React-Renderer).
      updatePatterns(
        ps => applySoloUpdate(ps, partId, soloed, exclusive),
        false
      );
  ...
  const setPartColor = useCallback(
    (partId: string, color: string | undefined) => {
  ...
  const setPartStepResolution = useCallback(
    (partId: string, res: StepResolution | undefined) => {
      updatePatterns(
        ps =>
          ps.map(p => ({
            ...p,
            parts: p.parts.map(pt =>
              pt.id === partId ? { ...pt, stepResolution: res } : pt
            ),
          })),
        false
      );
  ...
  const setPartStepLength = useCallback(
    (partId: string, length: number | undefined) => {
      updatePatterns(ps =>
        ps.map(p => ({
          ...p,
          parts: p.parts.map(pt => {
  ...
  const setPartFx = useCallback(
    (partId: string, fxUpdate: Partial<ChannelFx>) => {
      updatePatterns(
        ps =>
          ps.map(p => ({
            ...p,
  ...
  const setPartMicroTiming = useCallback(
    (partId: string, offsetMs: number) => {
      updatePatterns(
        ps =>
          ps.map(p => ({
            ...p,
            parts: p.parts.map(pt =>
              pt.id === partId
                ? { ...pt, microTiming: Math.max(-50, Math.min(50, offsetMs)) }
                : pt
            ),
          })),
        false
  ...
  const setPartSourceType = useCallback(
    (partId: string, type: "sample" | "wavetable" | "fm" | "granular") => {
  ...
  const setPartStretchRatio = useCallback(
    (partId: string, ratio: number) => {
      updatePatterns(
        ps =>
          ps.map(p => ({
            ...p,
            parts: p.parts.map(pt =>
              pt.id === partId ? { ...pt, stretchRatio: ratio } : pt
            ),
          })),
        false
      );
  ...
  const setPartGranularParams = useCallback(
    (
      partId: string,
      params: Partial<import("../audio/GranularEngine").GranularParams>
    ) => {
              ...
              pt.id === partId
                ? {
                    ...
                    granularParams: {
                      ...(pt.granularParams ?? {}),
                      ...params,
                    } as import("../audio/GranularEngine").GranularParams,
                  }
                : pt
            ),
          })),
          ...
          ps.map(p => ({
            ...
            parts: p.parts.map(pt => {
              ...
              return {
                ...
                synthParams: patch.synthParams
                  ? { ...patch.synthParams }
                  : pt.synthParams,
                granularParams: patch.granularParams
                  ? { ...patch.granularParams }
                  : pt.granularParams,
  ...
  const setPartSteps = useCallback(
    (
      partId: string,
      newActive: boolean[],
      velocities?: number[],
  ...
  const setStepVelocity = useCallback(
    (partId: string, stepIndex: number, velocity: number) => {
      updatePatterns(
        ps =>
          ps.map(p => ({
            ...p,
  ...
  const setStepPitch = useCallback(
    (partId: string, stepIndex: number, pitch: number) => {
      updatePatterns(
        ps =>
          ps.map(p => ({
            ...p,
  ...
  const setStepProbability = useCallback(
    (partId: string, stepIndex: number, probability: number) => {
      updatePatterns(
        ps =>
          ps.map(p => ({
            ...p,
  ...
  const setStepSlide = useCallback(
    (partId: string, stepIndex: number, slide: boolean) => {
      updatePatterns(
        ps =>
          ps.map(p => ({
            ...p,
  ...
  const setStepCondition = useCallback(
    (partId: string, stepIndex: number, condition: StepCondition) => {
      updatePatterns(
        ps =>
          ps.map(p => ({
            ...p,
  ...
  const setStepLength = useCallback(
    (partId: string, stepIndex: number, length: number) => {
      updatePatterns(
        ps =>
          ps.map(p => ({
            ...p,
  ...
  const setStepChainNext = useCallback(
    (
      partId: string,
      stepIndex: number,
      chain: "up" | "down" | "none" | undefined
  ...
  const setStepParamLock = useCallback(
    (
      partId: string,
      stepIndex: number,
      lock: import("../audio/AudioEngine").StepParamLock | undefined
  ...
  const setStepReverse = useCallback(
    (partId: string, stepIndex: number, reverse: boolean) => {
      updatePatterns(
        ps =>
          ps.map(p => ({
            ...p,
  ...
  const setPartEuclidean = useCallback(
    (partId: string, hits: number, steps: number, rotation = 0) => {
  ...
  const resetAll = useCallback(() => {
    ...
    const fresh = JSON.parse(
      JSON.stringify(INITIAL_PATTERN)
    ) as typeof INITIAL_PATTERN;
  ...
  const shiftPattern = useCallback(
    (partId: string, direction: "left" | "right") => {
      updatePatterns(ps =>
        ps.map(p => {
          if (p.id !== state.activePatternId) return p;
          return {
  ...
  const applyStepCountToAllPatterns = useCallback(
    (count: StepCount) => {
      updatePatterns(ps =>
        ps.map(p => ({
          ...p,
  ...
  const setVelocityMode = useCallback((active: boolean) => {
    setState(prev => ({
      ...prev,
      velocityMode: active,
      pitchMode: active ? false : prev.pitchMode,
    }));
  }, []);
  ...
  const setPitchMode = useCallback((active: boolean) => {
    setState(prev => ({
      ...prev,
      pitchMode: active,
      velocityMode: active ? false : prev.velocityMode,
    }));
  }, []);
  ...
  const getPlaybackPattern = useCallback(() => {
    return state.patterns.find(
      p => p.id === (state.playbackPatternId ?? state.activePatternId)
    );
  ...
  return {
    ...
    copySamplesFromPattern,
    reorderPatterns,
    startLivePatternEdit,
    commitLivePatternEdit,
    cancelLivePatternEdit,
    scheduleCommit,
    setPatternBpm,
    setPatternBpmRatio,
    setPatternBpmTransitionBars,
    setPatternStepResolution,
    setPatternFollowAction,
