...
const INDEX = {
  ...
  files: {
    ...
    "client/src/utils/electribeImport.ts (v3.12.0 Step-Encoding RE)": {
      role:     "v3.12.0: Step-Encoding für .e2spat Real-Files vollständig reverse-engineered. CORRECTED Part-Stride 896→816 (16 parts × 816B = 13056B ab 0x900, + 1280B trailing pattern-footer = 16640B File-Total). VERIFIED 12-byte step-record-Layout per Part-Block (ab part_off+0x30, 64 records): byte0=Trigger(0x00/0x01), byte1=Velocity(0..127 explicit / 0xFF=sentinel→127 default), byte2=0x60 const, byte3=Accent/Tied(0/1, TBD), byte4=MIDI-Note(0x48=C5 default), bytes5..11=reserved. parseRealPartBlock komplett umgebaut: liest Trigger+Velocity+defensive-clamp out-of-range velocity (0x80..0xFE → 127). RE-Methodik: Hex-Diff Init181 (1024 identische 12-byte records) vs BodyTalk1 (programmed); positions[64]-positions[0]=0xC61-0x931=816 confirms part-stride. Verification gegen 4 real files: BodyTalk1 Part 6 [2,6,10,...,62] = offbeat hi-hat, Part 11 [0,4,12,...,60] = 4-beat kick exact match. Confidence: ✅ HIGH (Trigger+Velocity+Note+Stride+StepCount), ⚠ MEDIUM (Part-Header Volume/Pan/Pitch/FxSend Offsets), ❌ LOW (Motion-Seq, StepLength, Swing). v3.12-Konstanten: ELECTRIBE_REAL_PART_HEADER_BYTES=0x30, _STEP_RECORD_BYTES=12, _STEPS_PER_PART=64, _STEP_TRIGGER/_VELOCITY/_NOTE_OFFSET=0/1/4, _VELOCITY_DEFAULT_SENTINEL=0xFF/_VALUE=127. v3.11.0-API unverändert: +Multi-Pattern-Bank-Support fuer .e2sallpat (250 Slots, ~4 MB Stock-Banks). Verified Bank-Layout: File-Header 0x000..0x100 identisch zu .e2spat (KORG + e2sampler + Version u32 LE + 0xFF padding) + Bank-Header bei 0x100 ('GLST' Magic 4B + u32 LE Chunk-Length 256 + 248B Metadata mostly-zero + 'GLED' End-Marker bei 0x1FC) + Padding 0x200..0x10100 (0xFF) + 250 Pattern-Records ab 0x10100 mit Stride 16384B (0x4000). Pro Pattern-Record: PTST+0x00=Marker(4B) + PTST+0x10=Name(16B ASCII) + PTST+0x22=BPM×10 (u16 LE) + PTST+0x800=Parts-Offset (PTST-relativ statt file-absolut 0x900 wie bei .e2spat). File-Total exakt 4 161 792 Bytes. NEU Public-API: parseElectribeAllPatBank(buf)→ParsedElectribeBank{version,patternCount=250,patterns[]}, isElectribeAllPatBank(buf)→bool (KORG+e2sampler+GLST+PTST-Detection), detectElectribeFormatKind(buf)→'e2spat'|'e2sallpat'|'legacy'|'unknown', filterNonInitPatterns(patterns[])→ParsedPattern[] (filtert 'Init Pattern' + 'Slot N' Fallbacks). NEU intern parseRealPatternAt(view,ptstOffset,slotIndex) als generischer PTST-relativer Parser — parseRealPattern fuer .e2spat wurde zu duennem Wrapper (ruft parseRealPatternAt mit ptstOffset=0x100, slotIndex=1). Eliminiert Code-Duplikation zwischen Single- und Multi-Pattern-Pfaden. detectElectribeFormat erweitert um e2sallpat-Branch (liefert 'bank'). parseElectribeBank dispatched .e2sallpat → parseElectribeAllPatBank. Defensive gegen kaputte PTST-Marker: Fallback-Pattern 'Slot N' + Default-BPM 120 + leerer parts-Array statt Throw. MAX_ELECTRIBE_FILE_BYTES 5MB → 8MB. NEU Konstanten: ELECTRIBE_ALLPAT_FIRST_PATTERN_OFFSET=0x10100, _PATTERN_STRIDE=0x4000, _SLOT_COUNT=250, _EXPECTED_SIZE=4_161_792, _GLST_MARKER='GLST', _GLST_OFFSET=0x100, _GLED_MARKER='GLED'. v3.2.0 .e2spat-API unveraendert: parseElectribeBank/Pattern/convertParsedPatternToSynthstudio/looksLikeElectribeFile/isRealElectribeFile + Real-Layout-Konstanten ELECTRIBE_REAL_* + Legacy/Synthetic-Layout fuer Tests.",
      lastSeen: "2026-05-18T11:05:00.000Z",
      ownedBy:  "backend"
    },
