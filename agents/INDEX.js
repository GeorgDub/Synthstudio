...
const INDEX = {
  ...
  files: {
    ...
    "client/src/components/KorgBank/KorgBankEditor.tsx (v3.6.0)": {
      role:     "v3.6.0: WRITE-Side KORG E2S Sample-Bank-Editor (~400 LOC). Komplementär zu KorgBankModal (Read-Only). Props {open:bool, onClose}. Left-Panel: Pick-Liste aus useProjectStore.samples (Click → addSampleAsSlot, auto-slotIndex=next.length, name auto aus Filename). Right-Panel: Slot-Editor pro Row (Name-Input max 16 chars, Category-<select> mit E2S_CATEGORY_NAMES, Oneshot-Checkbox, Frame-Count/Status, Remove-Button). Settings-Row oben: Target-SR-Dropdown 44100/48000, Force-Mono-Checkbox, Filename-Input. Footer: Total-Bytes-Display + Cancel + 'Als .all speichern'-Button (gated PRO_FEATURE_KORG_BANK_WRITE). v3.6 handleSaveAs ruft buildE2sBank(inputs, {preserveRawRiff:true}) auf und setzt isDirty:true pro picker-Slot (neue Samples sind immer 'dirty'). Bei zukünftigem Bank-Reload-Flow (v3.6-FOLLOWUP-1) können unedited Slots aus geladener .all-Bank mit rawRiff durchgereicht werden → Faktory-Bank bleibt bit-exakt. v3.6 nutzt automatisch polyPhase-Resampling via convertToE2sSpec default. Tests via data-testids korg-bank-editor-{overlay,close,picker,pick-*,list,row-*,name-*,cat-*,remove-*,target-sr,force-mono,filename,status,cancel,save}.",
      lastSeen: "2026-05-18T09:35:00.000Z",
      ownedBy:  "frontend"
    },
