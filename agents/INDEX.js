...
const INDEX = {
  ...
  files: {
    ...
    "client/src/utils/korg/esxBankEditorState.ts (v3.29.0)": {
      role:     "v3.29.0 NEU: Pure-TS Editor-State-Model fuer den ESX-Pattern-Patch-Mode im KorgBankEditor (~165 LOC, isomorph, KEINE Electron/DOM/AudioEngine-Deps). Wraps eine geladene .esx-Bank (ArrayBuffer ~24-28MB) + 256 Slot-Overview-Rows + pendingPatches: Map<index, ArrayBuffer 4280B>. Public API: buildEsxSlotOverview(bank, totalSlots=256) → 256 dense EsxSlotRow{index, empty, name, bpm, stepLength} (parser skipt leere Slots — Overview synthesisiert empty-Placeholder), stageEsxPatch(map, index, block) → new Map (immutable, last-write-wins bei Same-Slot, throws auf invalid index oder block!==4280B), unstageEsxPatch(map, index) → new Map (returns SAME ref wenn slot nicht im map, kein allocation), commitEsxPatches(bankBuffer, pending) → ArrayBuffer (sortiert pending-keys ascending fuer deterministic write-order, delegiert an patchEsxBankPatterns aus v3.28), hasPendingEsxPatches/countPendingEsxPatches helpers, filterEsxRows(rows, query, hideInit, hideEmpty) → Rows-Subset (Suche per Name-Substring case-insensitive ODER Index '5'/'005', hideInit skipt isInitPatternName-Matches, hideEmpty skipt empty slots), isInitPatternName(name) → boolean (trimmed lowercase: 'init pattern', 'init pat', 'init'). EsxEditorState type exportiert (sourceName, byteLength, rows, pendingPatches). Types EsxSlotRow + EsxEditorState public.",
      lastSeen: "2026-05-18T15:30:00.000Z",
      ownedBy:  "frontend"
    },
    "client/src/components/KorgBank/KorgBankEditor.tsx (v3.29.0)": {
      role:     "v3.29.0 ERWEITERT: ESX-Pattern-Patch-Mode hinzugefuegt als dritter Tab (zusaetzlich zu v3.4 'Neue Bank' + v3.7 'Bank bearbeiten' E2-Modi). Neue Props: getActiveSynthPattern?: () => SynthstudioPatternLike|null (Closure vom App-Root um dm.getActivePattern() zu lesen, ohne dm direkt zu importieren — Pure-Component-Boundary), globalBpm?: number (Fallback wenn pattern.bpm===null). Neuer Mode 'esx' im EditorMode-Union. tryChangeMode handelt drei outgoing-modes (new/edit/esx) mit confirm-Dialog bei dirty-state. Neuer Workflow: 'ESX Patterns'-Tab → '📂 ESX-Bank öffnen' (hidden file-input accept='.esx,.ESX') → parseEsxBank(buffer) → buildEsxSlotOverview → 256 Slot-Liste (LinkRow: 2/5 width). Filter: case-insensitive Suche (Name oder #index) + 'Init verbergen' + 'Leere verbergen' toggles. Pro Slot: '↻ Mit aktuellem Pattern ersetzen'-Button → convertSynthstudioPatternToEsx + buildEsxPatternBlock → stageEsxPatch (rote-Dot dirty-Indicator). Pre-Save confirm-Dialog: 'X Slots geändert, Rest bit-exakt — Save?'. Save: commitEsxPatches → electron.saveEsxBankAs (native Save-Dialog) ODER Blob-Download (Browser-Fallback). loadBankFromFile (.all → edit mode) routet jetzt .esx → loadEsxBankFromFile via filename-Sniff. resetOnClose nimmt auch ESX-State mit. data-testids: korg-bank-editor-mode-esx / -esx-open / -esx-open-input / -esx-open-another / -esx-search / -esx-hide-init / -esx-hide-empty / -esx-slot-list / -esx-slot-<i> / -esx-dirty-<i> / -esx-detail / -esx-replace / -esx-revert / -esx-filename / -esx-status / -esx-save.",
      lastSeen: "2026-05-18T15:30:00.000Z",
      ownedBy:  "frontend"
    },
