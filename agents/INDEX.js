...
const INDEX = {
  ...
  files: {
    ...
    "client/src/components/KorgBank/KorgBankEditor.tsx (v3.7.0)": {
      role:     "v3.7.0: Korg E2S Sample-Bank-Editor mit Mode-Toggle Neue Bank/Bank bearbeiten (~740 LOC). v3.6-Funktion (Pick-Liste aus useProjectStore.samples + Slot-Editor + Save-As) bleibt als mode='new' erhalten. NEU mode='edit': '📂 Bank öffnen'-Button → hidden <input type=file accept=.all> → file.arrayBuffer() → parseE2sBank({preserveRawRiff:true}) → bankToOpenedSlots → setOpenedSlots. Slot-Browser links (md:w-2/5): 250-Row-Liste, empty als '—Empty—' dim italic, filled mit Name+Category+Dirty-Dot, klickbar zur Selektion (data-testid=korg-bank-editor-slot-N + korg-bank-editor-dirty-N). Slot-Detail rechts: Name-input (max 16), Category-select (alle 18 E2S_CATEGORY_NAMES), Oneshot+Gain12dB Checkboxes, SampleTune Range (-99..+99), '🎵 Sample ersetzen'-Button → hidden <input audio/*> → decodeArrayBuffer+convertToE2sSpec+replaceSlotSample, '🗑 Löschen'-Button → deleteSlot, '↺ Revert'-Button (disabled wenn !isDirty) → revertSlot. Audio-Info-Panel (channels/sr/frames). Empty-Detail zeigt nur Revert. tryChangeMode mit window.confirm-Gate bei unsaved changes (hasUnsavedChanges aus bankEditorState). handleSaveAs branched: mode='new' = legacy decoder+buildE2sBank(isDirty:true pro picker-Slot), mode='edit' = openedSlotsToBuildInputs()+buildE2sBank({preserveRawRiff:true}) — Toast mit 'N geändert, M bit-exakt erhalten'. NEU externalOpenFile/onExternalOpenFileConsumed-Props für Drag-Drop-Integration aus App.tsx (auto-load wenn .all gedroppt während Editor offen). Pro-Feature-Gate (PRO_FEATURE_KORG_BANK_WRITE) auch auf Open-Bank-Flow. data-testids: korg-bank-editor-{overlay,close,mode-toggle,mode-new,mode-edit,open,open-input,open-another,replace-input,slot-browser,slot-N,dirty-N,detail,detail-empty,detail-name,detail-category,detail-oneshot,detail-gain12db,detail-tune,replace-sample,revert,delete,picker,pick-*,list,row-*,name-*,cat-*,remove-*,target-sr,force-mono,filename,status,cancel,save}.",
      lastSeen: "2026-05-18T09:50:00.000Z",
      ownedBy:  "frontend"
    },
    "client/src/utils/korg/bankEditorState.ts (v3.7.0)": {
      role:     "v3.7.0: Pure-isomorphe State-Helpers für den KorgBankEditor's Open-Bank-Flow (~310 LOC, keine React/DOM-Deps). OpenedSlot Type: rowId, slotIndex (0..249), empty:bool, name, category, oneshot, gain12db, sampleTune, pcmData?/sampleRate?/channels?/frames?, rawRiff?:Uint8Array, isDirty:bool, original:OpenedSlotSnapshot|null. Public API: bankToOpenedSlots(bank,prefix?) konvertiert E2sBank.slots[250] → 250-row Editor-Repräsentation (empty:true für offset=0-Slots, isDirty=false, original-Snapshot kopiert pcmData/sampleRate/channels/frames/rawRiff für Revert, oneshot abgeleitet aus src.loopType===LOOP_TYPE_ONESHOT). patchOpenedSlot(slots,rowId,patch) immutable update, setzt isDirty=true wenn editable-Felder (name/category/oneshot/gain12db/sampleTune oder pcmData/sampleRate/channels) sich tatsächlich ändern (Wertvergleich — kein flip bei no-op). replaceSlotSample(slots,rowId,pcm,sr,ch) fully fills slot incl empty→filled, frames=pcm.length/channels, immer isDirty=true. deleteSlot(slots,rowId) empty:true + isDirty:true + dropped rawRiff/pcmData/sampleRate/channels, original-Snapshot bleibt für Revert. revertSlot(slots,rowId) restored aus original-Snapshot inkl rawRiff-Referenz, isDirty=false; ohne original (urspr. empty) macht wieder empty. openedSlotsToBuildInputs(slots) → {inputs:E2sSlotInput[], dirtyCount, passthroughCount, droppedCount}: empty Slots werden skipped (no entry → offset=0), filled-aber-PCM-fehlt droppt defensive mit Counter, isDirty/rawRiff propagated zur buildE2sBank-API für preserveRawRiff. Helpers: countFilledSlots, countDirtySlots, hasUnsavedChanges, displayName ('—Empty—' / '(unnamed)' / name), displayCategory (aus E2S_CATEGORY_NAMES).",
      lastSeen: "2026-05-18T09:50:00.000Z",
      ownedBy:  "frontend"
    },
    ...
    "client/src/utils/electribeImport.ts": {
      role:     "TASK-237 (v2.88.0) + TASK-237-CALIBRATION (v3.2.0): Browser-safe KORG Electribe 2 Pattern-Parser mit Dual-Layout-Support. KEIN Web-Audio-Import. Public-API: parseElectribeBank/parseElectribePattern/convertParsedPatternToSynthstudio/detectElectribeFormat/looksLikeElectribeFile/isRealElectribeFile/clamp01/clampPan. v3.2.0 NEU: Real-File-Layout verified gegen 4 echte KORG E2 Sampler-Files: (0x00 16B Magic 'KORG'+zeros) + (0x10 16B 'e2sampler'+zeros) + (0x20 u32 LE Version=1) + (0x24..0xFF 0xFF padding) + (0x100 16B 'PTST'+zeros Pattern-Marker) + (0x110 16B Pattern-Name ASCII) + (0x122 u16 LE BPM×10) + (0x900 16 Parts × 896 Bytes = total 16640 Bytes). Auto-Detection via isRealElectribeFile (KORG@0x00 + e2sampler@0x10 + PTST@0x100) routet zu parseRealPattern; sonst Legacy-Pfad. parseRealPartBlock liefert Best-Effort-Defaults (Vol=100/Pan=64/Pitch=0/FxSend=0/Steps=inactive/Motion=disabled) bis Step-Encoding reverse-engineered ist. Verified Fields (Real): Magic+ID+Marker, Name, BPM. Best-Effort (Real): StepLength, Swing, Part-Header-Offsets, Step-Trigger-Encoding, Motion-Slots. Legacy/Synthetic-Layout (v2.88-Spec) für Tests unverändert: Magic+Version+Count Bank-Header (8 Bytes LE), Pattern-Block 16+16*PartBlock Bytes, Part-Block 8+64+4*MotionSlot Bytes, Step-Byte Bit7=active+Bits0-6=velocity. BPM fixed-point /10 mit Clamp 20..300. StepLength 64 → 32. Constants: ELECTRIBE_MAGIC='KORG', ELECTRIBE_REAL_IDENTIFIER='e2sampler', ELECTRIBE_REAL_PATTERN_MARKER='PTST', ELECTRIBE_REAL_FILE_SIZE=16640, ELECTRIBE_REAL_NAME_OFFSET=0x110, ELECTRIBE_REAL_BPM_OFFSET=0x122, ELECTRIBE_REAL_PARTS_OFFSET=0x900, ELECTRIBE_REAL_PART_STRIDE=896, MAX_PATTERNS_PER_BANK=250, PARTS_PER_PATTERN=16, STEPS_PER_PART=64, MOTION_SLOTS_PER_PART=4, MOTION_STEPS_PER_SLOT=16, MAX_ELECTRIBE_FILE_BYTES=5MB. SafeReader-Helper mit ensure()-Bounds-Guard.",
      lastSeen: "2026-05-18T08:00:00.000Z",
      ownedBy:  "backend"
    },
    "tests/features/electribe-import.test.ts": {
      role:     "TASK-237 (v2.88.0) + TASK-237-CALIBRATION (v3.2.0): 64 Unit-Tests fuer den Electribe-Parser + Konverter. Legacy/Synthetic-Coverage (42 Tests): Magic/Format-Detection (6), Pattern-Header BPM/Name/StepLength/Swing (9), Parts+Steps Velocity-Bit-Decode/Pitch-Signed/Motion-Slots (8), Bank-File-Reading (3), Defensive Out-of-Bounds + 5MB-Limit (4), Synthstudio-Konvertierung (8), Motion-Sequencer → AutomationLanes (4). v3.2.0-Real-Layout-Coverage (+22 Tests): isRealElectribeFile-Detector (6 — positive/Legacy-rejection/KORG-zerstört/e2sampler-zerstört/PTST-zerstört/zu-klein), synthetische Real-Layout-Parser (10 — BPM 120, BPM 165 BodyTalk1, Name 'Advi$ory1', Name 'Init Pattern' mit Space, 16 Parts mit Defaults, BPM-Clamp-Garbage, detect→pattern, Version u32 LE aus 0x20, Layout-Konstanten 16×896==14336==total-0x900, convert→Synthstudio). Real-File-Coverage (6 conditional auf REAL_FILES_AVAILABLE describe.skip): 245_BodyTalk1.e2spat-Header (16640+Magic+BPM=165+Name='BodyTalk1'), 181_Init=120BPM, 250_Init=170BPM, 001_Advisory1=128BPM+Name='Advi$ory1', 4-File-Magic-Roundtrip, convert→Synthstudio mit echtem BodyTalk1. buildRealElectribeBuffer-Builder baut synthetisch das echte Layout nach (mit 0xFF-Padding 0x24-0xFF). buildElectribeBuffer-Helper für Legacy-Layout unverändert. Alle 64 gruen.",
      lastSeen: "2026-05-18T08:00:00.000Z",
      ownedBy:  "backend"
    },
