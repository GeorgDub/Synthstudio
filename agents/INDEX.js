...
const INDEX = {
  ...
  files: {
    ...
    "client/src/components/KorgBank/KorgBankEditor.tsx (v3.8.0)": {
      role:     "v3.8.0: Korg E2S Sample-Bank-Editor mit Mode-Toggle Neue Bank/Bank bearbeiten + NEU inline ESLI Slice-Editor (~1170 LOC). v3.7-Funktionen unverändert. NEU v3.8: import sliceBridge (onsetsToEsliSlices/esliSlicesToOnsets/MAX_ESLI_SLICES) + autoSlice + WaveformSliceCanvas + setSlotSlices. NEU extractMonoChannel(pcm,ch) Helper am Modul-Top (Stereo→Mono-Visualisierung via Kanal 0). NEU Methods editSlotSetSlices/handleAutoSlice/handleClearSlices. NEU renderSliceEditor(slot) am Ende des Slot-Detail-Panels (mode='edit'): '✂ Slices (N/64)'-Header + 'Auto-Slice'-Button (autoSlice mit maxSlices=64, fillToMax=false, snapToZero=true → OnsetCandidate[] → onsetsToEsliSlices(onsets, totalFrames) → setSlotSlices) + 'Clear'-Button (slices=[]) + <WaveformSliceCanvas channelData={mono} sampleRate=… onsets onChange=… maxSlices=64 height=120 snapToZero=true> + Help-Text. data-testids: korg-bank-editor-slice-editor/-auto/-clear/-canvas/-count. handleSaveAs unverändert — der bestehende preserveRawRiff-Pfad funktioniert mit Slice-Edits, weil patchOpenedSlot bei slices-Patch isDirty=true flippt → Builder re-encoded den Slot inkl. Slices bei 0x58. Pro-Feature-Gate weiterhin auf Open-Bank-Flow.",
    ...
    "client/src/components/KorgBank/WaveformSliceCanvas.tsx (v3.8.0)": {
      role:     "v3.8.0 NEU: Inline-Waveform + Slice-Marker-Komponente für KorgBankEditor (~280 LOC). Extrahiert die Canvas-Render-Logik aus SampleSliceEditor (v2.89), aber als controlled-Komponente (Props: onsets+onChange-Callback, Caller hält State). Render: peak-reduzierter Waveform (min/max pro Bucket, O(N) buildPeaks), Center-Line, vertikale Slice-Marker mit nummeriertem Index-Tag, Drag-Highlight (markerDragColor). Theme-aware via getComputedStyle(--ss-bg-elevated/--ss-text-muted/--ss-border/--ss-accent-primary/--ss-accent-secondary). RAF-rendering mit devicePixelRatio-Scaling. ResizeObserver für responsive Canvas-Breite (defensive: typeof ResizeObserver === 'undefined' guard für Node). Pointer-Interaktion: Linksklick auf leere Stelle = addOnset (cap maxSlices), Drag = moveOnset, MouseUp = snapToZeroCrossing (256-Frame-Radius) wenn snapToZero=true, Shift/Rechtsklick auf nahegelegenen Marker (~4px tolerance) = removeOnset. Props: channelData, sampleRate, onsets, onChange, maxSlices=64 (default), height=120 (Mini-Inline vs 200 Modal), snapToZero=true, testId, className. Verwendet snapToZeroCrossing/addOnset/moveOnset/removeOnset aus sampleSlicing.ts.",
      lastSeen: "2026-05-18T10:05:00.000Z",
      ownedBy:  "frontend"
    },
