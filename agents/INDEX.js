...
const INDEX = {
  ...
  files: {
    "client/src/utils/korg/audioProcessor.ts (v3.4.0)": {
      role:     "v3.4.0: Pure-TypeScript Audio-Processor für E2S-Export (~250 LOC). Port aus Korg Editor/esx_e2s_editor/services/audio_processor.py. Public API: convertToE2sSpec(pcm, sr, ch, opts) → ProcessedAudio (resample auf 44100|48000, optional forceMono via average-downmix, optional peakNormalize). floatToInt16LeBytes(pcm) Float32→16-bit-LE-Bytes (clip [-1,+1], NaN→0, +1.0→0xFF7F, -1.0→0x0080). downmixToMono(stereo) (L+R)/2 pro Frame + Post-Peak-Return. resampleLinear(pcm, inSr, outSr, ch) — MVP lineare Interpolation (kein Anti-Alias, Quality-Followup v3.5). peakNormalize(pcm, target ∈ (0,1]) — silent-input passthrough. sanitizeE2sSlotName(name, maxLen=16) ASCII-printable-Filter (0x20..0x7E). AudioProcessError sealed-class. Defensive: targetSampleRate ∈ E2S_SAMPLE_RATES, per-Slot-Cap MAX_BYTES_PER_SLOT=10MB, NaN/Infinity-Filter im float→i16-Pfad.",
      lastSeen: "2026-05-18T08:50:00.000Z",
      ownedBy:  "backend"
    },
    "client/src/utils/korg/e2sBankBuilder.ts (v3.4.0)": {
      role:     "v3.4.0: Pure-TypeScript E2S `.all`-Sample-Bank-Builder (~340 LOC). Port aus Korg Editor/esx_e2s_editor/services/e2s_builder.py (743 LOC). Public API: buildE2sBank(slots: E2sSlotInput[]) → BuildResult ({buffer:ArrayBuffer, slotCount, warnings}). E2sSlotInput-Felder: slotIndex(0..249), name (16-char ASCII, getrimmt), category(0..17 clamped), pcmData:Float32Array, sampleRate(44.1k/48k), channels(1|2), loopType, loopStart/EndBytes, level(0..127), gain12db, sampleTune(-99..+99), slices (max 64×{start,length,attack,amplitude}), sliceSteps(max 64B raw), slicingNumSteps/Beat/NumActive. Layout-Output identisch zu Python: 0x0000 16B 'e2s sample all\\x1a\\0' signature, 0x07E0 250×LE32 offset-table, 0x1000 RIFF-area. Pro Slot: RIFF + WAVE + fmt(16B) + data(pcm+word-pad) + korg(1180B). korg-Body: 'esli'+LE32-0x0494+LE16-0x01F4 + Name@0x0A + Cat@0x1A + PlayVolume@0x2C (skaliert level [0..127] → u16 [0..0xFFFF]) + LoopStart@0x34 + LoopEnd@0x38 + Oneshot@0x3C + UseChan0@0x48 + UseChan1@0x49 + Plus12dB@0x4A + Freq@0x50 + Tune@0x55-i8 + Slices@0x58 + SliceSteps@0x458 + Slicing-Meta@0x498..0x49A. Caps: max 250 Slots (E2sBuildError), max 10MB PCM/Slot, max 224MB cumulative PCM (E2S_MAX_TOTAL_PCM_BYTES), max 512MB total file. Duplicate-slotIndex: keep-first + warn. clampU8/clampU32 für Slice-Felder. estimatePcmBytesForSlot exported für UI-Form-Validation. Bit-Layout verified via Round-Trip-Test (buildE2sBank → parseE2sBank → input identical).",
      lastSeen: "2026-05-18T08:50:00.000Z",
      ownedBy:  "backend"
    },
    "client/src/components/KorgBank/KorgBankEditor.tsx (v3.4.0)": {
      role:     "v3.4.0: WRITE-Side KORG E2S Sample-Bank-Editor (~400 LOC). Komplementär zu KorgBankModal (Read-Only). Props {open:bool, onClose}. Left-Panel: Pick-Liste aus useProjectStore.samples (Click → addSampleAsSlot, auto-slotIndex=next.length, name auto aus Filename). Right-Panel: Slot-Editor pro Row (Name-Input max 16 chars, Category-<select> mit E2S_CATEGORY_NAMES, Oneshot-Checkbox, Frame-Count/Status, Remove-Button). Settings-Row oben: Target-SR-Dropdown 44100/48000, Force-Mono-Checkbox, Filename-Input. Footer: Total-Bytes-Display + Cancel + 'Als .all speichern'-Button (gated PRO_FEATURE_KORG_BANK_WRITE). handleSaveAs decodes all pending samples (Blob-URL fetch → AudioContext.decodeAudioData → convertToE2sSpec) sequentiell, dann buildE2sBank → IPC saveKorgBankAs (Electron) ODER Blob-Download mit URL.createObjectURL (Browser). Lazy AudioContext im Ref, close-on-unmount. Slot-Sanitization: 16-char-Maxlength im Name-Input. Tests via data-testids korg-bank-editor-{overlay,close,picker,pick-*,list,row-*,name-*,cat-*,remove-*,target-sr,force-mono,filename,status,cancel,save}. Semantic Tailwind classes (bg-bg-panel/elevated, accent-primary/danger, border-border-color).",
      lastSeen: "2026-05-18T08:50:00.000Z",
      ownedBy:  "frontend"
    },
