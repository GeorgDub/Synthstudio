...
const INDEX = {
  ...
  files: {
    ...
    "tests/features/midi-clock-in.test.ts (v3.37.0)": {
      role:     "v3.37.0 ERWEITERT: v3.36 32 Tests + NEU 15 v3.37-Tests = 47 Tests gesamt. NEU 3 describes: (1) 'v3.37 SPP throttle' (6 Tests: leading-edge immediate, 100-Events-Burst max-3-dispatches bounds, trailing-edge-last-value, widely-spaced no-throttling, 0xFA cancels trailing, disable cancels timer). (2) 'v3.37 0xFA with positionStep' (2 Tests: SPP-then-FA → positionStep=48, no-SPP → positionStep=0). (3) 'formatPatternPosition' (7 Tests: step 0/5/15 no-loop, step 16/48 loop, 32-step pattern bar 2, defensive negative/NaN/zero-stepCount). NEU ManualScheduler-Test-Harness (deterministic setTimeout-Mock + advanceBoth-Sync mit now()) — Throttle-Bursts bit-genau steuerbar ohne vi.useFakeTimers. v3.36 + v3.35 Tests UNVERÄNDERT grün.",
      lastSeen: "2026-05-18T18:30:00.000Z",
      ownedBy:  "backend"
    },
    "client/src/audio/AudioEngine.ts (v3.37.0 pendingStartStep)": {
      role:     "v3.37.0 ERWEITERT: v3.36 seekToStep + currentStepIndex Getter bleiben + NEU Race-Fix gegen play(0)-Overwrite. (a) NEU _pendingStartStep: number|null Field. seekToStep setzt jetzt _pendingStartStep=wrapped (zusätzlich zu _currentStep). (b) NEU consumePendingStartStep(): number|null — Single-use Konsumer für Tests/Inspektion. (c) NEU play(fromStep=0)-Verhalten: wenn fromStep===0 (Default Transport-Hook) UND pendingStartStep gesetzt → konsumiert pending statt 0 zu nutzen; explizit fromStep>0 bleibt unverändert. Damit Race aufgelöst: useTransport.play(0) liest automatisch SPP-Wert ohne Modify-Notwendigkeit. (d) NEU read-only Getter `stepCount` (parallel zu currentStepIndex) — für UI-Konsumenten (MidiSettings) die die Pattern-Länge brauchen ohne dm-Referenz. v3.35 _externalSyncActive + setBpm/applyExternalBpm/setExternalSyncActive unverändert.",
      lastSeen: "2026-05-18T18:30:00.000Z",
      ownedBy:  "backend"
    },
    "client/src/hooks/useMidi.ts (v3.37.0 onPlayStop signature)": {
      role:     "v3.37.0 ERWEITERT: v3.36 MidiClockIn-Wiring + SPP-Bridge bleiben + NEU onPlayStop-Signature-Erweiterung (Race-Fix). (a) UseMidiOptions.onPlayStop `() => void` → `(positionStep?: number) => void`. (b) onStart-Listener im midiclockin:start-Useeffect ruft `optionsRef.current.onPlayStop?.(pos)` mit positionStep aus Event-Detail (statt parameter-los + zusätzlichem seekToStep-Call). Der seekToStep-Call im Listener wurde ENTFERNT — Single-Source-of-Truth: useMidi reicht den Wert weiter, der Konsument (App.tsx-Wrapper) entscheidet. (c) onContinue/onStop rufen onPlayStop OHNE positionStep (Continue-Spec: resume from current). (d) midiclockin:spp-Listener weiter mit setClockInSpp + AudioEngine.seekToStep. (e) Backward-compatible: optionaler Parameter — alle 7+ Callsites bleiben kompatibel. v3.36 clockInSpp State + Return-Object unverändert.",
      lastSeen: "2026-05-18T18:30:00.000Z",
      ownedBy:  "backend"
    },
    ...
    "client/src/utils/patternPosition.ts (v3.37.0)": {
      role:     "v3.37.0 NEU: Pure-TS Helper für Bar.Beat.Sub-Display aus MIDI-Beat-/Step-Positionen (~75 LOC, isomorph, kein React/Electron). Public: formatPatternPosition(positionStep, stepCount): PatternPositionDisplay {bar, beat, sub, effectiveStep, isLooped, loopCount, label}. Zerlegt step in bar (1+floor/STEPS_PER_BAR), beat (1+floor((effective%16)/4)), sub (1+(effective%4)). Wenn positionStep ≥ stepCount: effectiveStep=step%stepCount, isLooped=true, loopCount=floor/stepCount, label suffix ' (loop)'. Defensive: negative/NaN-Steps → 0, stepCount≤0 → 16 fallback. Constants exportiert: STEPS_PER_BEAT=4, BEATS_PER_BAR=4, STEPS_PER_BAR=16.",
      lastSeen: "2026-05-18T18:30:00.000Z",
      ownedBy:  "backend"
    },
    "client/src/audio/MidiClockIn.ts (v3.36.0)": {
      role:     "v3.36.0: SUPERSEDED → siehe v3.37.0 Eintrag oben. v3.36-Verhalten bleibt erhalten (SPP-driven Pattern-Seek + pendingStartStep + 0xFA Event-Detail).",
    ...
    "tests/features/korg-esx-sample-patch.test.ts (v3.30.0)": {
      role:     "v3.30.0 NEU: 31 Tests in 7 describes (env:node, kein jsdom). buildSyntheticEsxBank-Helper baut .esx-Buffer mit allen Magics + 256 mono-Headers (alle empty) + 128 stereo-Headers (alle empty) + optional prefilled Mono-Slots (Name+Offsets+PCM-Bytes an deterministic Offsets). (1) Validators 4×: synthetic-bank valid + KORG-magic corrupt throw + Sample-Section-Magic corrupt throw + buffer-too-small throw. (2) Helpers 4×: encodeEsxName (pad+truncate+non-printable), float32ToBe16Pcm (BE bytes + clip + NaN→0 defensive), isSlotEmpty (mono+stereo). (3) Mono-Patch 4×: appends + updates header bytes/name/sampleRate/playLevel + Bank groesse-Inkrement, increments numMono-Counter wenn empty, behaelt Counter unveraendert wenn Slot bereits filled, updates currentOffset@0x1B0028 zum projected end-of-pcm. (4) Bit-Exact-Preservation via FNV-1a 6×: pattern-region 0x0200..0x130000 bit-exact, song-region 0x130000..0x1B0000 bit-exact, global 0x20..0x200 bit-exact, magic 0..0x20 bit-exact, andere sample-headers bit-exact bei single-slot-patch (FNV-Range check der Header-Bytes), andere PCM-Bytes bit-exact (prefilled slot 3's PCM unchanged bei Patch slot 25). (5) Validation-Errors 8×: out-of-range mono-index (256, -1), out-of-range stereo-index (128), invalid channels (3), empty PCM, oversized PCM > MAX_BYTES_PER_SLOT, stereo PCM odd length, invalid sampleRate (0, NaN), corrupt bank-buffer. (6) Round-Trip via parseEsxBank 3×: patch mono sine → parsed monoSamples[0] hat correct name/frames/sampleRate + PCM-Werte mit int16-Quantisation-Error < 0.001, prefilled OLD-slot bleibt bit-identisch nach patch OTHER slot, stereo-patch decodet als 2-channel interleaved. (7) Header-Offset-Helpers 2×: getEsxMonoHeaderOffset/getEsxStereoHeaderOffset match Layout + throws auf out-of-range/non-integer.",
      lastSeen: "2026-05-18T15:45:00.000Z",
      ownedBy:  "backend"
    },
    "client/src/utils/korg/esxBankCompacter.ts (v3.32.0)": {
      role:     "v3.32.0 NEU: Pure-TS ESX-1 Bank PCM-Compacter (~280 LOC, isomorph, KEINE Electron/DOM/AudioEngine-Deps). Closes Caveat 'Mode-A Orphan-Bytes-Waste' aus v3.30 esxSamplePatcher.ts. Public API: compactEsxBank(buffer: ArrayBuffer|Uint8Array) → ArrayBuffer (truncated to live-PCM-only) + inspectEsxBankWaste(buffer) → {liveBytes, pcmRegionBytes, orphanBytes, liveSlotCount}|null. Algorithm: (1) validateBankBufferForCompact prüft KORG-Magic@0x00 + ESX\\0-Submagic@0x08 + second-magic@0x1B0000 + Size-Range. (2) collectLivePcmRanges iteriert 256 mono- + 128 stereo-Headers, sammelt LivePcmRange[]: {srcRel, byteLength, headerKind:'mono'|'stereo-L'|'stereo-R', headerIndex}. Skip-Bedingungen: empty-offset (0xFFFFFFFF) ODER end<=start ODER len>MAX_BYTES_PER_SLOT ODER end>pcmRegionEnd. Deterministische Reihenfolge: mono 0..255 ascending, dann stereo 0..127 ascending mit L→R innerhalb jedes Slots. (3) newSize = 0x250000 + sum(rangeBytes). (4) Allocate new ArrayBuffer, copy header-region [0, 0x250000) bit-exact. (5) Re-allocate PCM ranges contiguous + update offset-fields in headers (mono +8/+12, stereo +8/+12 L + +16/+20 R). (6) currentOffset @0x1B0028 = newPcmTotal. BIT-EXACT GARANTIE: bytes [0, 0x250000) UNCHANGED (inkl. KORG/ESX magic, globals, alle 256 Pattern-Slots, song-data, sample-section-magic, counter, ALLE non-offset-Header-Felder name/sampleRate/sampleTune/playLevel/start/end/loopStart). WAS ÄNDERT: offset-Felder + currentOffset + Buffer-byteLength. Defensiv: throw EsxBankCompactError bei magic-corrupt/too-small/over-cap. inspectEsxBankWaste throws nicht — returnt null bei invalid input (UI-safe).",
      lastSeen: "2026-05-18T16:00:00.000Z",
      ownedBy:  "backend"
    },
    "tests/features/korg-esx-bank-compact.test.ts (v3.32.0)": {
      role:     "v3.32.0 NEU: 8 Tests in 2 describes (env:node, kein jsdom). buildSyntheticEsxBank-Helper aus korg-esx-editor-sample geforked. (1) Orphan removal 6×: Mehrfach-Re-Replace slot 5 → compactEsxBank entfernt orphans + LAST-name/frames bleibt + orphanBytes==0 + bank-byteLength shrinkt. Header-Inhalt preservt: name+start+end+loopStart+sampleRate+sampleTune+level bit-identical, off1Start/End MAY differ aber != EMPTY_OFFSET. Pattern-Region [0x200..0x130000) FNV-1a bit-exact + Global-Region [0x20..0x200) + Song-Region [0x130000..0x1B0000). 0-orphan input (file trimmed to live-only) → output bit-identical full-file. Round-Trip 3 samples + 1 re-replace → parseEsxBank vor/nach Compact: gleiche Indices, gleicher name+frames+sampleRate, PCM toBeCloseTo(6) (Bytes-Move ohne Re-Quantize). Validation: tooSmall + corrupt-magic throw EsxBankCompactError + inspectEsxBankWaste returns null. (2) Stereo round-trip 1×: 2 stereo-slots (idx 0/5) + re-replace slot 0 → parseEsxBank nach Compact zeigt beide stereo samples mit korrekten names (STER3/STER2) + frames (48/32) + channels==2.",
      lastSeen: "2026-05-18T16:00:00.000Z",
      ownedBy:  "backend"
    },
    "client/src/utils/korg/esxBankEditorState.ts (v3.32.0)": {
      role:     "v3.32.0 ERWEITERT: v3.31 Mono-Sample-API bleibt + NEU Stereo-Sample-API (~650 LOC gesamt). EsxStereoSampleSlotRow{index 0..127 LOCAL, empty, name, channels:2, sampleRate, frames, level} — der Parser nutzt index=256+i, wir normalisieren beim Build. buildEsxStereoSampleSlotOverview(bank, totalSlots=128) → 128 dense rows (parser stereoSamples ist sparse). stageEsxStereoSamplePatch(map, slot 0..127, entry) → new Map (validates channels===2 + pcmData even length für L,R interleaved + slot 0..127 + valid sampleRate). unstageEsxStereoSamplePatch (SAME ref bei unknown). hasPendingEsxStereoSamplePatches / countPendingEsxStereoSamplePatches helpers. commitEsxStereoSamplePatches(bankBuffer, pending) → ArrayBuffer (sortiert keys ascending, iteriert durch patchEsxBankSample mit channels:2 — wirft wenn entry.channels !== 2). filterEsxStereoSampleRows analog filterEsxSampleRows. v3.31 commitEsxPatchesAll bleibt unverändert (patterns+mono); UI ruft commitEsxStereoSamplePatches separat nach commitEsxPatchesAll. Design: stereo-pending ist eine SEPARATE Map<0..127, EsxSamplePatchEntry> — NICHT in die mono-Map gemerged, weil mono-slot 5 und stereo-slot 5 unterschiedliche Header-Tabellen sind.",
      lastSeen: "2026-05-18T16:00:00.000Z",
      ownedBy:  "backend"
    },
    "tests/features/korg-esx-editor-sample.test.ts (v3.32.0)": {
      role:     "v3.32.0 ERWEITERT: v3.31 17 Tests + NEU 10 Stereo-Tests = 27 Tests gesamt (env:node, kein jsdom). NEU stereo-Tests in 3 describes: (1) buildEsxStereoSampleSlotOverview 2× — 128 empty rows aus leerer Bank (alle channels===2), seeded slot 7 'STEREO' 128 frames @44100 via patchEsxBankSample channels:2 + parseEsxBank → row 7 non-empty mit korrektem name/sampleRate/frames. (2) stageEsxStereoSamplePatch 5× — stages immutable + countPendingEsxStereoSamplePatches===1, rejects index 128/-1/1.5, rejects channels!==2 (mono entry submitted), rejects odd-length pcmData (101 floats für interleaved L,R), unstage returns SAME ref bei unknown. (3) commitEsxStereoSamplePatches 3× — Stereo-Replace decoded L+R contiguous (NICHT interleaved on disk): off1End==off2Start, off2Start+frames*2==off2End verified via DataView, Round-Trip via parseEsxBank liefert pcmData[frames*2] interleaved mit beiden Channels toBeCloseTo(3) zum Original, filterEsxStereoSampleRows hideEmpty + 'bass'-search + '2'-index-match.",
      lastSeen: "2026-05-18T16:00:00.000Z",
      ownedBy:  "backend"
    },
    "client/src/components/KorgBank/KorgBankEditor.tsx (v3.32.0)": {
      role:     "v3.32.0 ERWEITERT: v3.31 Sub-Tab (Patterns|Samples) bleibt + NEU Mono/Stereo Channel-Mode-Toggle innerhalb des Samples-Tabs + NEU Compact-Bank-Action (~2480 LOC). NEU State: esxSampleChannelMode 'mono'|'stereo' + esxStereoSampleRows + esxStereoSamplePending Map + esxStereoSampleSelectedSlot + esxStereoSampleReplaceInputRef + esxStereoSampleReplaceTargetSlotRef. loadEsxBankFromFile parsed buildEsxStereoSampleSlotOverview parallel zu Mono. Mono/Stereo-Toggle-Pill ('Mono (N) | Stereo (M)' inline links neben Samples-Sub-Tab) zeigt filled-Counts. Channel-Mode dispatcht renderEsxSamplesBody (v3.31) vs renderEsxStereoSamplesBody (NEU v3.32). renderEsxStereoSamplesBody mirror der Mono-Body mit 128-Slot-Liste + 'S#'-Prefix + 'L+R'-Channel-Display + 'Mit Stereo-WAV ersetzen…'-Button. decodeWavForEsxStereoSample (NICHT-downmix): Mono-Input → L=R dupliziert, Stereo-Input → L+R-Channels einzeln resampled via polyPhaseResample(channel, src, 44100, 1), dann zu interleaved L,R für patchEsxBankSample channels:2. handleEsxStereoSampleReplaceClick/Input/replaceEsxStereoSampleSlot/handleEsxStereoSampleRevertSlot mirror der Mono-Handlers. NEU handleEsxCompactBank: inspectEsxBankWaste → confirm-Dialog 'Spare X.YZ MB' → compactEsxBank → re-parse + rebuild Overview-Rows + clear sample-pending-Maps (Pattern-Pending bleibt da Pattern-Offsets fix sind). Compact-Button im Sub-Tab-Header (only visible wenn esxSubTab='samples' + waste>0) zeigt Real-Bytes-MB. Header-Subtitle erweitert: 'NM/MO Smp' + 'X.Y MB Waste' Suffix bei waste>0. handleEsxSaveBank applies jetzt 3 pending-Maps: commitEsxPatchesAll (patterns+mono) → commitEsxStereoSamplePatches (stereo), confirm-Dialog summarisiert 3 Counts, alle 3 pending nach save geleert. tryChangeMode / reset-on-close cleanen auch stereo-state. data-testids: korg-bank-editor-esx-sample-channel-toggle / -channel-mono / -channel-stereo / -compact / -stereo-sample-* parallel zu mono.",
      lastSeen: "2026-05-18T16:00:00.000Z",
      ownedBy:  "frontend"
    },
    "client/src/utils/korg/esxBankEditorState.ts (v3.31.0)": {
      role:     "v3.31.0 ERWEITERT: v3.29 Pattern-Patch-State unverändert + NEU Sample-Patch-State (~360 LOC gesamt, isomorph, KEINE Electron/DOM/AudioEngine-Deps). Public Pattern-API (v3.29): buildEsxSlotOverview / stageEsxPatch / unstageEsxPatch / commitEsxPatches / filterEsxRows / isInitPatternName / EsxSlotRow / EsxEditorState. NEU v3.31 Sample-API: EsxSampleSlotRow{index, empty, name, channels:1|2, sampleRate, frames, level} + EsxSamplePatchEntry{pcmData:Float32Array, sampleRate, channels:1|2, name, level?}. buildEsxSampleSlotOverview(bank, totalSlots=256) → 256 dense EsxSampleSlotRow (parser monoSamples ist sparse — synthesisiert empty-Placeholders). stageEsxSamplePatch(map, slot 0..255, entry) → new Map (immutable, last-write-wins, throws auf invalid slot ODER leerer pcmData ODER channels !==1|2 ODER invalid sampleRate). unstageEsxSamplePatch(map, slot) → new Map oder SAME ref bei unknown slot. hasPendingEsxSamplePatches / countPendingEsxSamplePatches helpers. commitEsxSamplePatches(bankBuffer, pending) → ArrayBuffer (sortiert keys ascending fuer deterministic order, iteriert sequentiell durch patchEsxBankSample weil jede Patch die Bank GROESSER macht — chained pipeline; gibt fresh-copy bei empty Map). commitEsxPatchesAll(bankBuffer, patternMap, sampleMap) → ArrayBuffer (composite: ERST patterns slot-replace [fixed-size, Pattern-Region 0x200..0x130000), DANN samples append [grows buffer am Ende]). filterEsxSampleRows(rows, query, hideEmpty) → Subset (Suche per Name-Substring case-insens. ODER Index '5'/'005'). formatSampleLength(frames, sampleRate) → 'M:SS.ms' (0:01.000 für 44100 frames @44100 Hz) oder '—' für 0 frames oder '<N> fr' bei ungueltigem sampleRate (Fallback).",
      lastSeen: "2026-05-18T15:45:00.000Z",
      ownedBy:  "frontend"
    },
    "client/src/components/KorgBank/KorgBankEditor.tsx (v3.31.0)": {
      role:     "v3.31.0 ERWEITERT: v3.29 ESX-Mode behält 3 EditorModes (new/edit/esx) + NEU Sub-Tab-Toggle innerhalb des ESX-Modes ('Patterns' | 'Samples Mono'). EsxSubTab-Union 'patterns' | 'samples' im local State. Sub-Tab-Toggle rendert über dem 2-Spalten-Body mit Pending-Counter-Badges (●N) auf beiden Sub-Tab-Buttons + Hint 'Stereo-Samples folgen in v3.32'. NEUE State-Felder: esxSampleRows / esxSamplePending: Map<slot, EsxSamplePatchEntry> / esxSampleSelectedSlot / esxSampleSearch / esxSampleHideEmpty / esxSampleDropTargetSlot / esxSampleReplaceInputRef / esxSampleReplaceTargetSlotRef. handleEsxSaveBank refactored: nutzt jetzt commitEsxPatchesAll statt commitEsxPatches → save iteriert beide Patch-Maps (Patterns first, Samples danach). Confirm-Dialog summarisiert 'X Pattern-Slot(s) + Y Sample-Slot(s) wurden geändert'. Save-Button disabled wenn beide pending leer (esxTotalPendingCount===0). NEU handleEsxSampleReplaceClick → hidden file-input accept='.wav,.aiff,.aif,.mp3,audio/*'. decodeWavForEsxSample(file) → audioContext.decodeAudioData → Downmix Stereo→Mono (L+R)/2 framewise → polyPhaseResample(mono, sourceSr, 44100, 1) [ESX-1 Target-Rate, via existing audioProcessor.ts] → returns {pcm, sampleRate:44100, channels:1, name}. replaceEsxSampleSlot(slot, file) → stageEsxSamplePatch + Update Overview-Row. handleEsxSampleRevertSlot → unstageEsxSamplePatch + re-parse Bank für andere noch-staged Rows. handleEsxSampleDragOver/DragLeave/Drop pro <li> → WAV-Drop direkt auf Slot triggert Auto-Replace. NEU renderEsxSamplesBody: 2-Spalten-Layout. Links 2/5 Mono-Sample-Slot-Liste mit Search + Empty-Hide-Toggle, pro Slot: padded Index (#003), Name oder '—Empty—' (italic+dim), Length (M:SS.ms), rote-Dot dirty-Indicator. Drag-Over highlight border-accent-success. Rechts Slot-Detail (Name + Channels + Sample-Rate Hz + Länge + Level/127 read-only) + '🎵 Mit WAV-Datei ersetzen…'-Button + Revert-Button + Filename-Input. Reset-on-Close + tryChangeMode cleanen auch ESX-Sample-State. data-testids: korg-bank-editor-esx-subtab-toggle / -esx-subtab-patterns / -esx-subtab-samples / -esx-sample-replace-input / -esx-sample-search / -esx-sample-hide-empty / -esx-sample-list / -esx-sample-slot-<i> / -esx-sample-dirty-<i> / -esx-sample-detail / -esx-sample-replace / -esx-sample-revert / -esx-sample-filename / -esx-sample-status / -esx-sample-no-selection.",
      lastSeen: "2026-05-18T15:45:00.000Z",
      ownedBy:  "frontend"
    },
    "tests/features/korg-esx-editor-sample.test.ts (v3.31.0)": {
      role:     "v3.31.0 NEU: 17 Tests in 5 describes (env:node, kein jsdom). buildSyntheticEsxBank-Helper baut .esx-Buffer mit Magics + 256 mono-Headers + 128 stereo-Headers + optional prefilled Mono-Sample-Slots (deterministic offsets) + optional pattern-blocks an slot N. (1) buildEsxSampleSlotOverview 2× — 256 dense empty rows aus leerer Bank, mit 1 prefilled mono slot 3 'KICK' 44100Hz 32 frames bleiben slot 2/255 empty. (2) stage/unstageEsxSamplePatch 4× — stages immutably + counts pending, replaces existing patch (last-write-wins) beim 2. stage same slot, unstage returns SAME ref bei unknown slot (kein Allocation), throws auf invalid slot (-1/256/1.5) ODER invalid sampleRate (0) ODER empty pcmData ODER invalid channels (3 as 1). (3) commitEsxSamplePatches 3× — Single-Patch slot 5 sine 256 frames @44100 → Bank wächst um exakt 512 Bytes (256 × 2) + Round-Trip parseEsxBank zeigt 'BASS' name + 44100 Hz + 256 frames, Multi-Patch slots 0/10/100 in REVERSE Stage-Order → ascending write-order verifiziert via parsed monoSamples (s0='A'@64f, s10='B'@96f, s100='C'@128f), Empty pending → fresh-copy mit FNV-1a-identical bytes (returned !== input ref). (4) commitEsxPatchesAll 4× — applies pattern+sample patches in 1 Pass (Pattern slot 5 → 'NEW' BPM 175 + Sample slot 10 → 'SND' 128 frames, Buffer growth > original byteLength), only-samples-empty-patterns, only-patterns-empty-samples (Buffer-Size unverändert), full byte-identity with commitEsxPatches when sample-map ist empty (FNV-1a hash equal). (5) filterEsxSampleRows + formatSampleLength 4× — hideEmpty hides empty rows, case-insens. name-substring search ('kick'/'snare'/'HI'), exact index match ('5'/'005'/'100'), formatSampleLength 44100/44100→'0:01.000' + 22050/44100→'0:00.500' + 132300/44100→'0:03.000' + 0/44100→'—' + 100/0→'100 fr' (fallback bei ungueltigem sr).",
      lastSeen: "2026-05-18T15:45:00.000Z",
      ownedBy:  "frontend"
    },
    "electron/main.ts (v3.29.0)": {
      role:     "v3.29.0: +ipcMain.handle 'esx:save-bank-as' (suggestedFilename, data: number[]|Uint8Array|ArrayBuffer) → {success, filePath, bytesWritten}|{success:false, error}. STRIKT: validateEsxBankSaveFilename → Normalize → 16B-prefix → validateEsxBankBuffer (Min 0x00250010 Max 64MB + KORG@0x00 + ESX\\0@0x08) → dialog.showSaveDialog (filters [{name:'KORG ESX-1 Bank', extensions:['esx']}]) → final path.extname check (.esx) → fs.writeFile. +'esx:get-bank-save-cap' returnt ESX_BANK_SAVE_MAX_BYTES=64*1024*1024. Path-Traversal-Vektor durch Renderer eliminiert (Pfad aus Dialog). v3.4 korg:save-bank-as (.all) + v3.26 electribe:save-pattern (.e2spat) unveraendert.",
      lastSeen: "2026-05-18T15:30:00.000Z",
      ownedBy:  "frontend"
    },
    "electron/preload.ts (v3.29.0)": {
      role:     "v3.29.0: +saveEsxBankAs(suggestedFilename, data: ArrayBuffer|Uint8Array) → Promise<{success, filePath?, bytesWritten?, error?}> (Datenformat: Uint8Array via Array.from-Transport ueber IPC) + getEsxBankSaveCap. v3.26 saveE2Pattern + v3.4 saveKorgBankAs Bridges unveraendert.",
      lastSeen: "2026-05-18T15:30:00.000Z",
      ownedBy:  "frontend"
    },
    "electron/useElectron.ts (v3.29.0)": {
      role:     "v3.29.0: +saveEsxBankAs Browser-Fallback (returns success:false → Editor faellt auf Blob-Download zurueck) + getEsxBankSaveCap async-mirror (64MB). API-Delegation im Electron-Mode: saveEsxBankAs/getEsxBankSaveCap aus api ?? browserAPI. v3.4 saveKorgBankAs / v3.26 saveE2Pattern unveraendert.",
      lastSeen: "2026-05-18T15:30:00.000Z",
      ownedBy:  "frontend"
    },
    "electron/types.d.ts (v3.29.0)": {
      role:     "v3.29.0: +saveEsxBankAs(suggestedFilename, data) + getEsxBankSaveCap() im ElectronAPI-Interface. Mirror der v3.4 saveKorgBankAs + v3.26 saveE2Pattern Signaturen.",
      lastSeen: "2026-05-18T15:30:00.000Z",
      ownedBy:  "frontend"
    },
    "client/src/App.tsx (v3.29.0)": {
      role:     "v3.29.0: korg:bank:open Handler erweitert — bei offenem KorgBankExportOpen werden NUN sowohl .all (E2-edit mode) ALS AUCH .esx (ESX-patch mode) in den Editor durchgereicht (statt in den Read-Only KorgBankModal). KorgBankEditor erhält zwei neue Props: getActiveSynthPattern={() => dmRef.current.getActivePattern() ?? null} + globalBpm={project.bpm} — der Editor liest das aktuelle Pattern als pure SynthstudioPatternLike (kein dm-Import in der Editor-Komponente). v3.7 setKorgBankEditorFile + onExternalOpenFileConsumed unveraendert.",
      lastSeen: "2026-05-18T15:30:00.000Z",
      ownedBy:  "frontend"
    },
    "tests/features/korg-esx-editor.test.ts (v3.29.0)": {
      role:     "v3.29.0 NEU: 19 Tests in 6 describes (env:node, kein jsdom). (1) buildEsxSlotOverview 2×: 256 dense rows mit 3 seed patterns @ slots 0/1/5 (rows 2/3 empty placeholders, row 5 SEED5 BPM 175), all-empty rows wenn parseEsxBank patterns=[]. (2) stage/unstageEsxPatch 5×: stages immutably + counts pending, replaces existing patch beim 2. stage same slot, unstage returns SAME ref bei unknown slot, throws auf invalid index (-1/256/1.5), throws bei 4279/4281-byte blocks. (3) commitEsxPatches Bit-Exact 3×: Single-Patch slot 5 → all 255 anderen Slots FNV-1a-identical + Round-Trip parseEsxBank zeigt PATCH5 BPM 175, Multi-Patch slots 0/100/255 in Reverse-Stage-Order → deterministic ascending write-order via parse, Empty pending → fresh-copy with identical bytes (committed !== input ref). (4) isInitPatternName 2×: matches 'Init Pattern' / 'INIT PATTERN' / 'init pattern' / 'Init Pat' / 'init' / trimmed-whitespace + ignores 'KICK' / 'MyInitial' / ''. (5) filterEsxRows 4×: hideEmpty hides empty rows, hideInit hides 'Init Pattern'+'init' but keeps empties, case-insensitive name substring 'snare', exact index match '5' or '005'. (6) End-to-End Editor→Save flow 3×: convert→build→stage→commit→IPC-validators alle accept-flow (validateEsxBankSaveFilename + validateEsxBankBuffer auf committed 16B-prefix), IPC filename validator rejects unsafe variants (my.all/../etc/passwd.esx/a\\0b.esx/no_slash/in_name.esx/leer), parseEsxPattern of patched slot returns identical name+BPM+stepLength.",
      lastSeen: "2026-05-18T15:30:00.000Z",
      ownedBy:  "frontend"
    },
    "client/src/utils/korg/esxBankPatcher.ts (v3.28.0)": {
      role:     "v3.28.0 NEU: Pure-TS Bank Pattern-Patcher fuer KORG ESX-1 (~260 LOC, isomorph, KEINE Electron/DOM/AudioEngine-Deps). Public API: patchEsxBankPattern(bankBuffer: ArrayBuffer|Uint8Array, patternIndex: number 0..255, newPatternData: ArrayBuffer|Uint8Array of exactly 4280 bytes) → ArrayBuffer (same byteLength as input) + patchEsxBankPatterns(bankBuffer, patches: EsxBankPatch[]) für Multi-Slot-Replace in 1 Pass. Atomic-Semantik: Validate-all-then-write-all (2-Phase). Defensive: validateBankBuffer prueft KORG-Magic@0x00 + ESX\\0-Submagic@0x08 + Size-Range [ESX1_SIZE_FILE_MIN..ESX_FILE_MAX_BYTES=64MB] + EsxBankPatchError fuer alle Range/Magic-Verletzungen. Pro Patch geprueft: index Integer 0..255, data byteLength === ESX1_CHUNKSIZE_PATTERN (4280), Bank-Buffer groß genug für Ziel-Slot, Bulk-Duplicates via Set<index>. Output ist NEUER Buffer — Input wird NIE mutiert. Public Helpers: validateBankBuffer (throws EsxBankPatchError), looksLikeEsxBank (boolean), getEsxPatternSlotOffset(index) → 0x0200+i*4280, fnv1aHash32(bytes,start?,end?) FNV-1a 32-bit für efficient bit-exact verification via Math.imul. EsxBankPatch type: {index, data}. Spiegelt v3.6 e2sBankBuilder.preserveRawRiff-Pattern: slot-replace anstelle from-scratch fuer ~25MB Files. Scope: NUR Pattern-Slot-Replace (4280B chunks), NICHT sample/song/global edits.",
      lastSeen: "2026-05-18T15:00:00.000Z",
      ownedBy:  "backend"
    },
    "tests/features/korg-esx-bank-patch.test.ts (v3.28.0)": {
      role:     "v3.28.0 NEU: 29 Tests in 8 describes (env:node, kein jsdom). beforeEach builds synthetic-ESX-Bank with magic+sample-headers+pattern-slots. (1) Validators 4×: synthetic-bank valid, KORG-magic corrupt → throw EsxBankPatchError, ESX-submagic corrupt → throw, buffer below ESX1_SIZE_FILE_MIN → throw. (2) getEsxPatternSlotOffset 3×: slot 0 = 0x0200, slot 5 = 0x0200+5×4280, out-of-range -1/256/1.5 → throw. (3) Single-Slot-Patch 4×: overwrites exact 4280B at slot 5, byteLength preserved, ArrayBuffer+Uint8Array both accepted, input bank not mutated (fnv1aHash before/after identical). (4) Bit-Exact-Preservation via FNV-1a 5×: alle 255 anderen Pattern-Slots unchanged bei single-patch (per-slot Hash-Vergleich), Global-Region 0x20..0x200 bit-exact, Magic 0..0x20 bit-exact, Song-Region 0x130000..0x1B0000 bit-exact, Sample-Section 0x1B0000..EOF bit-exact. (5) Validation-Errors 5×: patternIndex -1/256/1.5 throw, wrong data-size 4279/4281 throw, invalid magic throw, duplicate bulk indices throw, non-array patches throw. (6) Bulk-Patch 2×: 3 patches at slots 0/100/255 each correctly applied + 253 untouched slots bit-exact, empty patches → identical-bytes copy NEW buffer. (7) CRITICAL Round-Trip via parser 2×: parseEsxBank seeded with 8 SEED-patterns → patch slot 5 → re-parse: slot 5 name='PATCH5' BPM=175, slots 0/1/2/3/4/6/7 byte-identical (per-slot FNV); parseEsxPattern of patched slot returns exact name+BPM+stepLength. (8) FNV-1a helper sanity 3×: identical bytes same hash, single-byte change different hash, start/end slice arguments honored. +1 conditional skip (real-file BOTTROP.ESX via fs.existsSync — fixture nicht im Repo).",
      lastSeen: "2026-05-18T15:00:00.000Z",
      ownedBy:  "backend"
    },
    "electron/ipcValidators.ts (v3.28.0)": {
      role:     "v3.28.0 ERWEITERT: NEU validateEsxBankSaveFilename (whitelist regex /^[A-Za-z0-9._-]+\\.esx$/ + NUL/slash/dotdot-defense + max 120 chars) + validateEsxBankBuffer (Min ESX_BANK_SAVE_MIN_BYTES=0x00250010 + Max ESX_BANK_SAVE_MAX_BYTES=64MB + KORG@0x00 prefix + ESX\\0@0x08 sub-magic prefix) + EsxBankFilenameCheck + EsxBankBufferCheck Union-Types. Separat von KORG_BANK_SAVE (.all = E2S) damit Endungs-Pflicht pro Format strikt enforced — kein Misch-Save zwischen ESX-1 und E2S. v3.26 E2_PATTERN + v3.4 KORG_BANK (.all) Validators unveraendert.",
      lastSeen: "2026-05-18T15:00:00.000Z",
      ownedBy:  "backend"
    },
    "client/src/utils/korg/esxPatternBuilder.ts (v3.27.0)": {
      role:     "v3.27.0 NEU: Pure-TS Binary-Builder fuer KORG ESX-1 Pattern-Blocks (~400 LOC, isomorph, KEINE Electron/DOM/AudioEngine-Deps). buildEsxPatternBlock(input: EsxPatternInput) → ArrayBuffer (immer EXAKT 4280 Bytes = ESX1_CHUNKSIZE_PATTERN-Constant aus korg/constants.ts geshared). Spiegelt v3.14/v3.20/v3.23 Read-Side (esxParser.ts) bit-genau. Pattern-Block-Layout: 0x000..0x007 8B Name ASCII space-padded (truncated >8 char), 0x008..0x009 BPM × 128 BE u16 (clamp 20..300, round), 0x00D StepLength-1 byte (default 0x0F=16 steps, range 1..64), 0x00F Swing 0..100, 0x018..0x163 10 Drum-Parts × 34B, 0x25C Stretch-Part 34B, 0x36E/0x38E/0x3AE/0x3CE 4 Short-Parts 32B. Drum/Stretch-Part-Layout (34B): +0..+1 sampleId BE u16 (0x8000 unassigned sentinel), +2..+3 'ff 00' invariant marker, +8 pitch (signed i8 around 0x40), +9 level, +10 pan, +11 fxSend, +18..+33 16 step-bytes. Short-Part-Layout (32B): +0..+1 sampleId, +6 pitch, +7 level, +8 pan, +10 fxSend, +16..+31 16 step-bytes. Step-Byte: bit 0 = active, bit 4 = accent. Defensive clampInt ueberall (BPM/level/pan/fxSend/pitch/swing/sampleId 9-bit mask). Public sub-exports: writeDrumPartBlock/writeShortPartBlock + encodeStepByte/encodePitchByte/encodeSampleId pure helpers + looksLikeEsxPatternBlock-Validator. Constants exported: ESX_PATTERN_BLOCK_SIZE=4280, ESX_DRUM_PARTS_OFFSET=0x18, ESX_DRUM_PART_STRIDE=34, ESX_STRETCH_PART_OFFSET=0x25C, ESX_SHORT_PART_OFFSETS=[0x36E,0x38E,0x3AE,0x3CE], alle Field-Offsets, ESX_SAMPLEID_UNASSIGNED=0x8000, ESX_PITCH_NEUTRAL_RAW=0x40, ESX_STEP_TRIGGER_BIT=0x01, ESX_STEP_ACCENT_BIT=0x10, ESX_ACCENT_VELOCITY_THRESHOLD=100. Types: EsxPatternInput/EsxDrumPartInput/EsxShortPartInput/EsxStepInput. Scope: NUR 4280B-Block, NICHT full 25MB-Bank (v3.28+).",
      lastSeen: "2026-05-18T14:30:00.000Z",
      ownedBy:  "backend"
    },
    "client/src/utils/korg/esxPatternConvert.ts (v3.27.0)": {
      role:     "v3.27.0 ERWEITERT: v3.5 Read-Convert (convertEsxPatternToSynthstudio) bleibt + NEU WRITE-Side. convertSynthstudioPatternToEsx(pattern: SynthstudioPatternLike) → EsxPatternInput. Mapping (1:1 partIndex 0..14): Synthstudio parts[0..9] → drumParts[0..9], parts[10] → stretchPart, parts[11..14] → shortParts[0..3], parts[15] → audioInPart. volume (0..1) → 0..127 level via synthVolumeToEsxLevel (round, undefined→100), pan (-1..+1) → 0..127 via synthPanToEsxPan (64+p*63, undefined→64), Synthstudio step.velocity > 100 → accent=true (ESX_ACCENT_VELOCITY_THRESHOLD const), pitch aus erstem aktiven Step mit definiertem pitch-Feld (best-effort, ESX hat nur 1 per-Part-Pitch, clamped -64..+63). Name truncated auf 8 Chars. BPM null/undefined → 120 Fallback. stepCount default 16 (Hardware-Spec). Structurally-compatible SynthstudioPatternLike/SynthstudioPartLike Types ohne AudioEngine-Dependency. Pure helpers exportiert: synthVolumeToEsxLevel/synthPanToEsxPan/synthStepToEsx/synthPartToEsxDrumPart/synthPartToEsxShortPart. Imports type-only von esxPatternBuilder.ts.",
      lastSeen: "2026-05-18T14:30:00.000Z",
      ownedBy:  "backend"
    },
    "tests/features/korg-esx-pattern-write.test.ts (v3.27.0)": {
      role:     "v3.27.0 NEU: 34 Tests in 9 describes (env:node, kein jsdom). (1) Builder-Basics 4×: 4280-Byte exact + looksLikeEsxPatternBlock-Validator + 4279-Byte rejection + 'ff 00' invariant marker. (2) Header-Fields 6×: Name space-padding short, truncate long >8, BPM=175 → 0x5780 BE u16, Range-Clamp [10→20, 999→300, 60.5 preserved], StepLength=16→0x0F + 32→0x1F, Swing 75 + clamp 200→100. (3) Drum-Part-Layout 5×: sampleId BE u16 + 0x8000 unassigned, pitch around 0x40 (0→0x40, -10→0x36, +12→0x4C), level/pan/fxSend offsets +9/+10/+11, clamp 500→127/-20→0/999→127, 4-on-the-floor steps via bit 0. (4) Step-Byte 4×: encodeStepByte inactive→0x00, active→0x01, active+accent→0x11 (bit 0|bit 4), inactive+accent→0x00. (5) Short-Part-Layout 3×: sampleId @0x36E, pitch/level/pan/fxSend offsets +6/+7/+8/+10, step-bytes @+16. (6) Stretch-Part 1×: 34B drum-layout @0x25C. (7) Encoding-Helpers 2×: encodePitchByte ±range + encodeSampleId 9-bit mask + 0x8000 unassigned. (8) CRITICAL Round-Trip 2×: 3-Drum/1-Stretch/1-Short programmiertes Pattern incl. accent → parseEsxPattern liefert identical name/bpm/lengthSteps/swing/parts[].sampleId/pitch/volume/pan/fxAmount/steps[].active/accent/velocity-127 + BPM stability across [60,100,120,128,140,175,200,240]. (9) Synthstudio-Adapter 7×: synthVolumeToEsxLevel scale + synthPanToEsxPan center-64 + synthStepToEsx velocity-threshold-100 + full convertSynthstudioPatternToEsx kick-pattern + truncated-name + parts-array-lengths + threshold-constant=100 + Synth→Adapter→Builder→Parse full round-trip kick-pattern BPM 140 + missing BPM fallback to 120.",
      lastSeen: "2026-05-18T14:30:00.000Z",
      ownedBy:  "backend"
    },
  ...
  workLog: [
    ...
    },
    {
      ...
      timestamp: "2026-05-18T18:30:00.000Z",
        ...
        "v3.37.0: SPP Race-Fix + Throttle + Pattern-Length-Display — closes v3.36 caveats (3 items). Closes: (a) onPlayStop-timing-race (0xFA → play(0) überschreibt seekToStep), (b) SPP-Throttle fehlt (DAW-Scrub-Bursts fluten Sequencer), (c) Pattern-Length > 16 fold display.",
        "RACE-FIX (Option A — onPlayStop-Signature-Erweiterung). GEÄNDERT client/src/hooks/useMidi.ts: UseMidiOptions.onPlayStop signature `() => void` → `(positionStep?: number) => void`. onStart-Listener im midiclockin:start-Useeffect ruft jetzt `optionsRef.current.onPlayStop?.(pos)` mit positionStep aus dem Event-Detail (statt parameter-los). Der bisherige `AudioEngine.seekToStep(pos)`-Call im Listener wurde ENTFERNT — Single-Source-of-Truth: useMidi reicht den Wert weiter, der Konsument (App.tsx) entscheidet was passiert. onContinue/onStop rufen onPlayStop OHNE positionStep (Continue-Spec: resume from current). Backward-compatible: bestehende Caller (CollabSplitView, ScriptRunner, useKeyboardShortcuts, Workspace) bleiben ohne Änderung — der optionale Parameter ist undefined.",
        "GEÄNDERT client/src/App.tsx: midi-Hook-onPlayStop von `project.togglePlayStop` zu wrapper `(positionStep) => { if (typeof positionStep === 'number') AudioEngine.seekToStep(positionStep); project.togglePlayStop(); }`. AudioEngine.seekToStep wird VOR togglePlayStop gerufen — damit setzt der Wrapper den _pendingStartStep BEVOR isPlaying flippt und useTransport.play(0) feuert. Die anderen 2 onPlayStop-Stellen (useKeyboardShortcuts, useMenuHandlers) bleiben unverändert — UI-Trigger erhalten keinen positionStep.",
        "GEÄNDERT client/src/audio/AudioEngine.ts: NEU `_pendingStartStep: number|null` Field + `consumePendingStartStep(): number|null` (single-use Konsumer). seekToStep setzt jetzt auch `_pendingStartStep = wrapped`. NEU `play(fromStep=0)`: wenn fromStep===0 (Default) UND pendingStartStep gesetzt → konsumiert pending statt 0 zu nutzen; explizit fromStep>0 bleibt unverändert. Damit ist der Race aufgelöst — useTransport.play(0) liest automatisch den letzten SPP-Wert. NEU read-only Getter `stepCount` (parallel zu currentStepIndex) für UI-Konsumenten die die Pattern-Länge brauchen ohne dm-Referenz.",
        "SPP-THROTTLE (50ms leading + trailing edge). GEÄNDERT client/src/audio/MidiClockIn.ts: NEU Constant `SPP_THROTTLE_MS=50` (=20 events/sec max). NEU private state `_sppLastDispatchTime`/`_sppTrailingPending`/`_sppTrailingTimer`. NEU private method `_emitSppThrottled(midiBeat)` — leading-edge: erstes Event sofort dispatched, folgende innerhalb 50ms werden zwischengespeichert; trailing-Timer feuert den LETZTEN Wert nach Ablauf. _onStart killt pending trailing-Timer (running-Gate). enable/disable/reset clean via `_resetSppThrottle()`. SPP-Handler im handleMidiMessage setzt pendingStartStep IMMER (auch bei suppressed dispatch) — finaler Wert vor 0xFA niemals verloren. NEU optional `scheduler` Option in MidiClockInOptions für Test-Injection (manual setTimeout/clearTimeout) — Default bindet globale.",
        "PATTERN-LENGTH-DISPLAY. NEU client/src/utils/patternPosition.ts (~75 LOC, pure isomorph, kein React/Electron). Public-API `formatPatternPosition(positionStep, stepCount): PatternPositionDisplay`. Zerlegt step in bar (1+floor/STEPS_PER_BAR), beat (1+floor((effective%16)/4)), sub (1+(effective%4)). Wenn positionStep ≥ stepCount: effectiveStep = step%stepCount, isLooped=true, loopCount=floor/stepCount, label endet auf ' (loop)'. Defensive: negative/NaN-Steps → 0, stepCount ≤ 0 → 16 fallback. Constants: STEPS_PER_BEAT=4, BEATS_PER_BAR=4, STEPS_PER_BAR=16.",
        "GEÄNDERT client/src/components/MidiSettings/MidiSettings.tsx: SPP-Display-Block ruft jetzt `formatPatternPosition(midi.clockInSpp, AudioEngine.stepCount)` und rendert `display.label`. Sub-Text differenziert: wenn isLooped → 'Step N/M · MIDI-Beat N · Loop X'; sonst 'Step N · MIDI-Beat N' (v3.36-Format). Import um AudioEngine + formatPatternPosition erweitert.",
        "NEU 15 Tests in tests/features/midi-clock-in.test.ts (47 total, vorher 32). 3 describes: 'v3.37 SPP throttle' (6 Tests: leading immediate, 100-events-burst max-3-dispatches bounds, trailing-edge-last-value, widely-spaced no-throttling, FA cancels trailing, disable cancels timer), 'v3.37 0xFA with positionStep' (2 Tests: SPP-then-FA → positionStep=48, no-SPP → positionStep=0), 'formatPatternPosition' (7 Tests: step 0/5/15 → no loop, step 16/48 → loop, 32-step pattern bar 2, defensive negative/NaN/zero-stepCount). NEU ManualScheduler-Test-Harness (deterministic setTimeout-Mock, sync mit now()) damit Throttle-Bursts bit-genau steuerbar ohne vi.useFakeTimers.",
        "TEST-RESULTAT: pnpm check clean (TypeScript strict ✓). pnpm test 186 files / 4314 tests passed (16 skipped, KEINE Regression vs v3.36 — vorher 4299, jetzt +15). midi-clock-in 47/47 GREEN.",
        "BACKWARD-COMPAT: useMidi-API erweitert (optionaler 2. Param) — alle 7+ Callsites (App.tsx, CollabSplitView, ScriptRunner, useKeyboardShortcuts, Workspace, useMenuHandlers) bleiben ohne Änderung kompatibel. AudioEngine.play(fromStep) Default-Verhalten unverändert wenn _pendingStartStep===null; explizite play(fromStep>0)-Calls bleiben unverändert. SPP-Event-Detail unverändert (midiBeat + positionStep). MidiSettings-UI bleibt visuell identisch wenn stepCount===16 ODER positionStep<stepCount (kein '(loop)'-Suffix).",
        "CAVEATS (verbleibend nach v3.37): (1) Externer Sync mit Pattern-Length > 16 nutzt jetzt die effective-step-Anzeige, aber der seekToStep moduloed weiter — d.h. ein DAW-Master der zu Bar 5 (step 80) springt, landet auf step 0 wenn der Synthstudio-Sequencer ein 16-step-Pattern hat. Bewusste Design-Entscheidung: kein Multi-Bar-Pattern-Konzept in Synthstudio bisher. (2) Trailing-Dispatch in der Default-Schedule-Variante nutzt window.setTimeout — bei Tab-Hidden (Background-Throttle) kann der trailing-Dispatch >50ms zu spät kommen. Akzeptabel da SPP nur außerhalb von Running gilt. (3) Optional v3.38: Toolbar-BPM-Slider explizit-disabled in External-Sync-Mode + Status-LED in Topbar (war schon v3.35/v3.36-Caveat).",
        "package.json + agents/INDEX.js version 3.36.0 → 3.37.0."
      ],
      next: [
        "Optional v3.38: Toolbar-BPM-Slider explizit disabled-State + Status-LED in Topbar (statt nur Settings) — bislang ist nur AudioEngine.setBpm geblockt, UI zeigt Werte ohne visuellen Hinweis.",
        "Optional v3.38: SPP-Display in der Topbar (nicht nur Settings) wenn external-sync running. Schöner UX-Feedback wenn der Master mid-track Position-Sprünge macht.",
        "Optional v3.38: Multi-Bar-Pattern-Support (z.B. 64-step Patterns) damit DAW-Master mit langen Song-Ranges 1:1 abgespielt werden ohne Loop-Faltung.",
        "Vorhandene v3.36-Items unverändert (Optional v3.38: ParsedPartStep accent+note, PTED Footer-Marker, Pattern-Header 0x120..0x200 RE)."
      ],
      changed: [
        "client/src/audio/MidiClockIn.ts (v3.37 SPP throttle leading+trailing 50ms + scheduler injection + _onStart cancels pending trailing)",
        "client/src/audio/AudioEngine.ts (NEU _pendingStartStep + consumePendingStartStep + play() konsumiert pending + stepCount getter)",
        "client/src/hooks/useMidi.ts (onPlayStop signature → (positionStep?: number) → void, onStart-Listener reicht positionStep durch statt seekToStep zu rufen)",
        "client/src/App.tsx (onPlayStop-Wrapper: seekToStep(pos) before togglePlayStop)",
        "client/src/utils/patternPosition.ts (NEU — formatPatternPosition Helper für Bar.Beat.Sub + Loop-Display)",
        "client/src/components/MidiSettings/MidiSettings.tsx (formatPatternPosition statt inline Bar-Calc + Loop-info subtitle)",
        "tests/features/midi-clock-in.test.ts (+15 v3.37-Tests, 32 → 47 GREEN, NEU ManualScheduler-Harness)",
        "package.json (3.36.0 → 3.37.0)",
        "agents/INDEX.js (version + workLog v3.37.0 + files-Einträge)"
      ]
    },
    {
      agent:     "backend",
    ...
    },
    {
      ...
      timestamp: "2026-05-18T16:00:00.000Z",
        ...
        "v3.32.0: ESX-1 Stereo-Sample Support + Compact-Action — closes v3.30/v3.31 Caveats. Schließt Caveat 'Nur Mono-Sample-Slots im UI/State' (v3.31) + Caveat 'Mode-A Orphan-Bytes-Waste bei Re-Replace' (v3.30) in einer Sitzung. Beide Features sind isomorph (pure-TS, keine Electron/DOM/AudioEngine-Deps in den Libraries) und Test-First implementiert (9 neue Tests insgesamt).",
        "STEREO LIBRARY-STATUS-CHECK: esxSamplePatcher.ts v3.30 hatte BEREITS vollen stereo-Pfad: channels:1|2-Parameter im EsxSamplePatchInput, 44B-Stereo-Header (vs 40B Mono), L/R-Split (NICHT interleaved) am append-Punkt mit off1Start=L-Block-Start, off2Start=L-Block-End, off2End=R-Block-End contiguous. PCM-Konvertierung float32ToBe16Pcm + BE-i16-Encoding gemeinsam mit Mono. Per-Slot-Cap (10MB) + cumulative-Cap (24MB) auch korrekt für stereo. NEU war NUR der State-Layer + UI.",
        "ERWEITERT client/src/utils/korg/esxBankEditorState.ts (~475 → ~650 LOC). NEU EsxStereoSampleSlotRow (parallel zu EsxSampleSlotRow aber mit channels:2 + index 0..127 LOCAL — Parser nutzt index=256+i, wir normalisieren). NEU buildEsxStereoSampleSlotOverview(bank, totalSlots=128) → 128 dense rows. NEU stageEsxStereoSamplePatch / unstageEsxStereoSamplePatch / hasPendingEsxStereoSamplePatches / countPendingEsxStereoSamplePatches / commitEsxStereoSamplePatches mirror der Mono-API. stageEsxStereoSamplePatch validates entry.channels===2 + pcmData.length%2===0 (interleaved L,R erforderlich) + slot 0..127. commitEsxStereoSamplePatches sortiert keys ascending + iteriert sequentiell durch patchEsxBankSample mit channels:2. filterEsxStereoSampleRows analog filterEsxSampleRows.",
        "NEU client/src/utils/korg/esxBankCompacter.ts (~280 LOC, pure-TS, isomorph). Public API: compactEsxBank(buffer) → ArrayBuffer (truncated to live-PCM-only) + inspectEsxBankWaste(buffer) → {liveBytes, pcmRegionBytes, orphanBytes, liveSlotCount} | null. Algorithm: (1) validate magic + second-magic, (2) collectLivePcmRanges() iteriert alle 256 mono- + 128 stereo-Headers (skip empty-offset/inverted/out-of-bounds/oversized), (3) sort ranges deterministic [mono-by-index, dann stereo-by-index mit L→R Reihenfolge], (4) allocate output buffer = ESX1_ADDR_SAMPLE_DATA + sum(rangeBytes), (5) copy header region [0, 0x250000) bit-exact, (6) re-allocate PCM ranges contiguous + update offset-fields in headers (mono +8/+12, stereo +8/+12 L + +16/+20 R), (7) currentOffset @0x1B0028 = newPcmTotal. WAS BLEIBT BIT-EXACT: [0, 0x250000) inkl. magic, globals, alle Pattern-Slots, song-data, sample-section-magic, sample-counter, ALLE non-offset-Header-Felder (name, sampleRate, sampleTune, playLevel, start, end, loopStart). WAS ÄNDERT SICH: offset-Felder + currentOffset + Buffer-byteLength.",
        "STEREO UI-WIRING client/src/components/KorgBank/KorgBankEditor.tsx (~2270 → ~2480 LOC). NEU State: esxSampleChannelMode 'mono'|'stereo' + esxStereoSampleRows + esxStereoSamplePending (Map<0..127, EsxSamplePatchEntry>) + esxStereoSampleSelectedSlot + esxStereoSampleReplaceInputRef + esxStereoSampleReplaceTargetSlotRef. loadEsxBankFromFile parsed jetzt ZUSÄTZLICH buildEsxStereoSampleSlotOverview + setEsxStereoSampleRows. Sub-Tab-Toggle erweitert: 'Samples' (nicht mehr 'Samples Mono') + inline Mono/Stereo Channel-Toggle-Pill (nur sichtbar wenn esxSubTab='samples') zeigt 'Mono (N)' und 'Stereo (M)' Counters. renderEsxStereoSamplesBody mirror von renderEsxSamplesBody mit 128-Slot-Liste + S#-Prefix. decodeWavForEsxStereoSample NICHT-downmix-Pfad: Mono-Input → L=R dupliziert, Stereo-Input → L+R separat, beide Channels einzeln resampled via polyPhaseResample(channel, sr, 44100, 1) dann zu interleaved L,R für patcher-Input.",
        "COMPACT UI-WIRING: handleEsxCompactBank() — confirm-Dialog 'Spare X Bytes (Y MB) durch Compaction?' + nach OK: compactEsxBank → re-parse → buildEsxSampleSlotOverview + buildEsxStereoSampleSlotOverview → setBuffer + clear sample-pending-Maps (Patterns bleiben weil deren offsets fix sind). Compact-Button im Sub-Tab-Header (only wenn esxSubTab='samples' + esxCompactReport.orphanBytes>0) zeigt 'X.YZ MB' inline. Header-Subtitle erweitert: 'MN/SO Smp' + bei waste 'N.M MB Waste' Suffix. handleEsxSaveBank applies jetzt patterns + monoSamples + stereoSamples sequentiell und clear alle 3 pending-Maps.",
        "TESTS STEREO 7× (in tests/features/korg-esx-editor-sample.test.ts erweitert auf 27 Tests gesamt): buildEsxStereoSampleSlotOverview empty-bank → 128 empty rows + seeded slot 7 'STEREO' 128 frames @44100 + andere bleiben empty. stage 4×: stages immutable + counts pending, rejects index 128/-1/1.5, rejects channels!==2 (mono-entry), rejects odd-length pcmData. unstage returns SAME ref bei unknown. commitEsxStereoSamplePatches 3×: Stereo-Replace produziert contiguous L+R (off1End==off2Start, off2Start+frames*2==off2End), Round-Trip via parseEsxBank liefert interleaved L,R im pcmData[2*frames] mit beiden Channels separat-decodet (toBeCloseTo(.001)), filterEsxStereoSampleRows hideEmpty + name-substring + index-match.",
        "TESTS COMPACT 8× (NEU tests/features/korg-esx-bank-compact.test.ts, env:node): (1) Mehrfach-Re-Replace slot 5 produziert orphan-Bytes → compactEsxBank entfernt sie, Bank-byteLength shrinkt, LAST-written name/frames bleibt, orphanBytes==0 nach Compact. (2) Header-Inhalt preserved: name+start+end+loopStart+sampleRate+sampleTune+level bleiben byte-identical, off1Start/End MAY differ aber Sind != EMPTY_OFFSET. (3) Pattern-Region [0x200..0x130000) FNV-1a-hash bit-exact, Global-Region [0x20..0x200) bit-exact, Song-Region [0x130000..0x1B0000) bit-exact. (4) 0-orphan input (file trimmed to live-only) → output bit-identical (FNV-1a full-file hash equal). (5) Round-Trip 3 live samples + 1 re-replace → parseEsxBank vor/nach Compact: gleiche Index-Set, gleicher name+frames+sampleRate, PCM toBeCloseTo(6) für jeden Frame (Compaction kopiert NUR Bytes, keine Re-Quantisation). (6) Validation-Errors: tooSmall + corrupt-magic throw EsxBankCompactError, inspectEsxBankWaste returns null statt throw. (7) Stereo round-trip: 2 stereo-slots + re-replace slot 0 → 2 stereo samples survive Compact mit korrekten Names + frames + channels==2.",
        "TEST-RESULTAT: pnpm check clean. pnpm test FULL: 184 files / 4245 passed + 16 skipped (war 183/4227 in v3.31 — +1 file/+8 tests Compact + +10 tests Stereo erweitert = +18 tests). KEINE Regression — alle existing KORG-Tests (v3.31 sample-editor + v3.30 sample-patch + v3.29 editor + v3.28 bank-patch + v3.27 pattern-write + v3.20/v3.23 parser/patterns) gruen.",
        "DESIGN-ENTSCHEIDUNG: Separate stereo-pending-Map statt unified Mono+Stereo-Map. Reasoning: mono-slot 5 und stereo-slot 5 sind verschiedene Entitäten (verschiedene Header-Tabellen). Eine unified Map<number, Entry> würde Konflikt erzeugen. Separate Maps geben User volle Freiheit (mono slot 5 + stereo slot 5 gleichzeitig stagen) und sind in commitEsxPatchesAll/commitEsxStereoSamplePatches separat angewendet — patcher kümmert sich um die korrekte Header-Tabelle via channels-Flag.",
        "DESIGN-ENTSCHEIDUNG Compact-Reihenfolge: Sample-Pending wird BEIM Compact gedropped weil deren append-Offsets relativ zum file-end SIND und das file-end ändert sich. Pattern-Pending bleibt weil Pattern-Slot-Offsets fix sind (0x200 + idx*4280). User-Warnung im Confirm-Dialog explizit. Alternative wäre staged-patches-rebasing — überaufwendig, würde State-Sync-Bugs erzeugen, dafür eindeutig die einfache UX 'compact-first oder save-first'.",
        "DESIGN-ENTSCHEIDUNG Stereo-Decode: NICHT mit decodeAudioData's OfflineAudioContext.startRendering() resamplen weil das mono-downmix forced + nicht-deterministic ist. Stattdessen explicit polyPhaseResample je channel separat → identische L+R-Length garantiert, deterministisch.",
        "CAVEATS: (a) Compact-Operation droppt staged sample-patches (bewusst — siehe Design-Entscheidung oben). User-Hinweis im Confirm-Dialog. (b) Compact ist eine 'destructive' Operation (in-memory) — Original-File wird erst beim nächsten Save überschrieben. Browser-Mode hat KEIN Auto-Backup. v3.33-Folgeaufgabe: 'Compact ↔ Undo' via Buffer-Snapshot-Stack im State. (c) Stereo-Sample-Listen-Cap ist 128 Slots — physische ESX-1-Hardware-Spec. Wenn User versucht slot 128 zu stagen → throw. (d) Stereo-Decode bei Mono-Input dupliziert die Channel-Daten 1:1 → die L+R-Frequencen sind identisch (kein faux-Stereo). Wenn User Pseudo-Stereo will, muss er via DAW vorab eine echte Stereo-WAV erzeugen. (e) Wave-Preview im Stereo-Detail fehlt noch — TODO v3.33 (siehe v3.31-Caveat WAVEFORM-PREVIEW). (f) Compact rebuildet PCM contiguous mit Mono-vor-Stereo-Reihenfolge. Falls Firmware-Bugs auf bestimmte slot-vs-PCM-Reihenfolge sensible sind (nicht beobachtet in real-files BOTTROP/KASSEL/ENDLICH/etc.), könnten Edge-Cases auftauchen. Empirisch: real-banks haben oft willkürliche Reihenfolge → Hardware verträgt das.",
        "package.json + agents/INDEX.js version 3.31.0 → 3.32.0."
      ],
      next: [
        "TASK-v3.32-FOLLOWUP-COMPACT-UNDO (v3.33): Buffer-Snapshot-Stack im KorgBankEditor — vor Compact einen Snapshot pushen + Undo-Button im Footer 'zurück zur pre-Compact-Bank'. State-Limit z.B. 3 Levels.",
        "TASK-v3.32-FOLLOWUP-WAVEFORM-PREVIEW (v3.33): Slot-Detail rechts (mono + stereo) zeigt WaveformSliceCanvas (existing v3.8) mit dem PCM. Stereo zeigt L-on-top, R-bottom (vertikaler Split).",
        "TASK-v3.32-FOLLOWUP-NAME-EDIT-INLINE (v3.33): Slot-Detail Name-Feld editable, patchEsxBankSampleHeader (nur 8B-Name in-place) ohne PCM-Replace.",
        "TASK-v3.32-FOLLOWUP-SAMPLE-AUDITION (v3.33): 'Play'-Button im Slot-Detail spielt das (replaced ODER existing) PCM ab via playSliceWithContext. Pre-listen vor Save.",
        "TASK-v3.32-FOLLOWUP-WORKER-RESAMPLE (v3.33): polyPhaseResample in WebWorker für lange Samples (>30s) damit Main-Thread nicht freezed.",
        "TASK-v3.32-FOLLOWUP-AUTO-COMPACT-ON-SAVE (v3.34): Optionale Settings-Checkbox 'Vor Save automatisch compactieren' damit User sich nicht selbst drum kümmern muss."
      ],
      changed: [
        "client/src/utils/korg/esxBankEditorState.ts (ERWEITERT v3.32: +EsxStereoSampleSlotRow + buildEsxStereoSampleSlotOverview + stageEsxStereoSamplePatch + unstageEsxStereoSamplePatch + hasPendingEsxStereoSamplePatches + countPendingEsxStereoSamplePatches + commitEsxStereoSamplePatches + filterEsxStereoSampleRows)",
        "client/src/utils/korg/esxBankCompacter.ts (NEU v3.32: compactEsxBank + inspectEsxBankWaste + EsxBankCompactError + ~280 LOC, pure-TS isomorph, defensiv validated)",
        "client/src/components/KorgBank/KorgBankEditor.tsx (ERWEITERT v3.32: +esxSampleChannelMode 'mono'|'stereo' + Mono/Stereo-Toggle-Pill + 4 stereo-State-Felder + decodeWavForEsxStereoSample [NICHT downmix] + handleEsxStereoSampleReplaceClick/Input + replaceEsxStereoSampleSlot + handleEsxStereoSampleRevertSlot + handleEsxCompactBank + renderEsxStereoSamplesBody + Compact-Button im Sub-Tab-Header + Header-Subtitle mit Waste-Display + handleEsxSaveBank applies stereo via commitEsxStereoSamplePatches nach commitEsxPatchesAll)",
        "tests/features/korg-esx-editor-sample.test.ts (ERWEITERT v3.32: 17 → 27 Tests, +10 stereo tests in 3 neuen describes: buildEsxStereoSampleSlotOverview 2× + stageEsxStereoSamplePatch 5× + commitEsxStereoSamplePatches 3×)",
        "tests/features/korg-esx-bank-compact.test.ts (NEU v3.32: 8 Tests in 2 describes — orphan removal 6× + stereo round-trip 1× + inspectEsxBankWaste 1×)",
        "package.json (3.31.0 → 3.32.0)",
        "agents/INDEX.js (version 3.31.0 → 3.32.0 + workLog v3.32.0 entry + neue files-Einträge)"
      ]
    },
    {
      agent:     "frontend",
      timestamp: "2026-05-18T15:45:00.000Z",
      done: [
        "v3.31.0: ESX-1 Sample-Tab im KorgBankEditor — UI wires v3.30 Library zur User-Action. Architektur-Entscheidung: Sub-Tab-Toggle innerhalb des bestehenden ESX-Modes (Patterns | Samples Mono) — KEIN dritter EditorMode. Reasoning: Sample-Edit ist eine ESX-Sub-Operation auf der gleichen Bank, kein eigenständiger Modus. Sub-Tab-Toggle rendert über dem 2-Spalten-Body und switcht zwischen renderEsxPatternsBody (v3.29) und renderEsxSamplesBody (NEU v3.31). Pending-Counter-Badges (●N) auf beiden Sub-Tab-Buttons machen Cross-Tab-Pending-State sichtbar.",
        "ERWEITERT client/src/utils/korg/esxBankEditorState.ts (~165 → ~360 LOC). NEU EsxSampleSlotRow{index, empty, name, channels:1|2, sampleRate, frames, level} + EsxSamplePatchEntry{pcmData, sampleRate, channels, name, level?}. NEU buildEsxSampleSlotOverview(bank, totalSlots=256) → 256 dense rows (parser monoSamples ist sparse — synthesisiert empty-Placeholders). stageEsxSamplePatch(map, slot, entry) → new Map (immutable + throws auf invalid slot oder leerer pcmData oder channels !==1|2 oder invalid sampleRate). unstageEsxSamplePatch(map, slot) → new Map oder SAME ref bei unknown slot. hasPendingEsxSamplePatches + countPendingEsxSamplePatches helpers. commitEsxSamplePatches(bankBuffer, pending) → ArrayBuffer (sortiert keys ascending, iteriert sequentiell durch patchEsxBankSample weil jede Patch die Bank GROESSER macht — chained pipeline). commitEsxPatchesAll(bankBuffer, patternMap, sampleMap) → ArrayBuffer (composite: ERST patterns slot-replace [fixed-size, Pattern-Region 0x200..0x130000), DANN samples append [grows buffer am Ende]). filterEsxSampleRows(rows, query, hideEmpty) → Subset (Suche per Name-Substring case-insens. ODER Index '5'/'005'). formatSampleLength(frames, sampleRate) → 'M:SS.ms' (0:01.000 für 44100 frames @44100 Hz) oder '—' für 0 frames oder '<N> fr' bei ungueltigem sampleRate.",
        "ERWEITERT client/src/components/KorgBank/KorgBankEditor.tsx (~1895 → ~2200 LOC): neuer EsxSubTab-Union 'patterns' | 'samples', neue State-Felder esxSampleRows + esxSamplePending + esxSampleSelectedSlot + esxSampleSearch + esxSampleHideEmpty + esxSampleDropTargetSlot + esxSampleReplaceInputRef + esxSampleReplaceTargetSlotRef. handleEsxSaveBank refactored: nutzt jetzt commitEsxPatchesAll statt commitEsxPatches → save iteriert beide Patch-Maps. Confirm-Dialog summarisiert 'X Pattern-Slot(s) + Y Sample-Slot(s) wurden geändert'. Save-Button disabled wenn beide leer (esxTotalPendingCount===0). NEU handleEsxSampleReplaceClick → File-Picker (hidden input accept='.wav,.aiff,.aif,.mp3,audio/*'), decodeWavForEsxSample → audioContext.decodeAudioData → Downmix Stereo→Mono (L+R)/2 → polyPhaseResample auf 44100 (ESX-1 Target-Rate, via existing audioProcessor.ts.polyPhaseResample), stageEsxSamplePatch + Update Overview-Row. handleEsxSampleRevertSlot → unstageEsxSamplePatch + re-parse Bank für andere noch-staged Rows (consistent mit v3.29 Pattern-Revert-Logic). handleEsxSampleDragOver/DragLeave/Drop → WAV-Drop direkt auf einen Slot triggert Auto-Replace (data-transfer-files-only). Reset-on-Close + tryChangeMode cleanen auch ESX-Sample-State.",
        "NEU renderEsxSamplesBody(): 2-Spalten-Layout. Links 2/5 Sample-Slot-Liste mit Search + Empty-Hide-Toggle, pro Slot: padded Index (#003), Name oder '—Empty—' (italic+dim), Length (formatSampleLength M:SS.ms), rote-Dot dirty-Indicator. Drag-Over highlight border-accent-success. Rechts Slot-Detail (Name + Channels [Mono/Stereo] + Sample-Rate Hz + Länge + Level/127 read-only), '🎵 Mit WAV-Datei ersetzen…'-Button, Revert-Button wenn dirty, Filename-Input für Save. Sub-Tab-Toggle-Header rendert über beidem mit Pattern-Tab ●N + Sample-Tab ●N + Hint 'Stereo-Samples folgen in v3.32'. KEINE hardcodierten Tailwind-Farben — alle semantic classes (bg-accent-primary, text-text-muted, border-border-color, text-accent-danger, etc.). ALLES isomorph — Drag-Drop via React's native handlers + decodeAudioData via window.AudioContext (web-Audio-API, kein Electron-Bridge nötig).",
        "TESTS NEU tests/features/korg-esx-editor-sample.test.ts (17 Tests in 5 describes, env:node, kein jsdom). (1) buildEsxSampleSlotOverview 2× — 256 dense empty rows aus leerer Bank, mit 1 prefilled mono slot (slot 3 KICK 44100Hz 32 frames) bleiben andere Slots empty. (2) stage/unstageEsxSamplePatch 4× — stages immutably + counts pending, replaces existing patch (last-write-wins) beim 2. stage same slot, unstage returns SAME ref bei unknown slot, throws auf invalid slot (-1/256/1.5) ODER invalid sampleRate (0) ODER empty pcmData ODER invalid channels (3). (3) commitEsxSamplePatches 3× — Single-Patch slot 5 sine 256 frames → Bank wächst um exakt 512 Bytes (256 × 2) + Round-Trip parseEsxBank zeigt 'BASS' name + 44100 Hz + 256 frames, Multi-Patch slots 0/10/100 (reverse stage order) → ascending write-order verifiziert via parsed monoSamples, Empty pending → fresh-copy mit FNV-1a-identical bytes (returned !== input ref). (4) commitEsxPatchesAll 4× — applies pattern+sample patches in 1 Pass (Pattern slot 5 → 'NEW' BPM 175 + Sample slot 10 → 'SND' 128 frames, Buffer growth > original), only-samples-empty-patterns, only-patterns-empty-samples (Buffer-Size unverändert), full identity with commitEsxPatches when sample-map ist empty (FNV-1a hash equal). (5) filterEsxSampleRows + formatSampleLength 4× — hideEmpty hides empty rows, case-insens. name-substring search ('kick'/'snare'/'HI'), exact index match ('5'/'005'/'100'), formatSampleLength 44100/44100→'0:01.000' + 22050/44100→'0:00.500' + 132300/44100→'0:03.000' + 0/44100→'—' + 100/0→'100 fr' (fallback).",
        "TEST-RESULTAT: pnpm check clean (3.30.0 → 3.31.0 version bump). pnpm test FULL: 183 files / 4227 passed + 16 skipped (war 182/4210 in v3.30 — +1 file/+17 tests). tests/features/korg-esx-editor-sample.test.ts: 17/17 passed. KEINE Regression — alle existing KORG-Test-Files (v3.30 korg-esx-sample-patch + v3.29 korg-esx-editor + v3.28 korg-esx-bank-patch + v3.27 korg-esx-pattern-write + v3.5/v3.20/v3.23 korg-esx-parser + v3.20 korg-esx-patterns) gruen.",
        "SAMPLE-RESAMPLING-STRATEGY: Browser-API audioContext.decodeAudioData liefert AudioBuffer mit nativer Sample-Rate (typisch 44100/48000/96000 je nach File). Wir downmixen Stereo→Mono explizit ((L+R)/2 frame-by-frame, NICHT decodeAudioData's eigenes Resampling-via-OfflineAudioContext) damit der Output deterministic ist. Dann polyPhaseResample(mono, sourceSr, 44100, 1) auf ESX-1-Target. polyPhaseResample ist Lanczos-3 + LPF (existing v3.6 audioProcessor.ts) — bei 48000→44100 ratio 1.088 gibt ~5ms CPU für 1s Audio. ESX-1 hardware-rate ist 44100 (canonical). v3.32-Stereo-Pfad würde polyPhaseResample(pcm, src, 44100, 2) für interleaved stereo nutzen.",
        "SAVE-PIPELINE-REIHENFOLGE: commitEsxPatchesAll → (1) commitEsxPatches(bankBuffer, patternMap) → patchEsxBankPatterns sequentiell (fixed-size 4280B slot-replace, Buffer-byteLength bleibt unverändert), (2) wenn samplePending.size > 0 → commitEsxSamplePatches(intermediate, sampleMap) → patchEsxBankSample sequentiell (jede Patch GROWS Buffer um neue PCM-Bytes). Patterns ZUERST weil sie fixe Offsets [0x200..0x130000) haben — sie können nicht mit Sample-Region-Writes kollidieren. Sample-Patches sind append-only ans Buffer-Ende (Mode A), also iterativ stackbar.",
        "DRAG-DROP-IMPL: <li>-Element pro Sample-Slot hat onDragOver (preventDefault + dropEffect='copy' + setDropTarget(slot)), onDragLeave (clear dropTarget), onDrop (preventDefault + clear dropTarget + dataTransfer.files[0] → replaceEsxSampleSlot(slot, file)). DropTarget-Slot rendert mit border-accent-success für visuelles Feedback. Kein zusätzlicher Drop-Zone-Wrapper, der Slot selbst ist der Drop-Target — schneller als 'drop anywhere → pick slot from dialog'.",
        "CAVEATS: (a) v3.31 fokussiert auf MONO-Slots (256 Stück, ca. 80% der ESX-Banks). Stereo-Tab (128 Slots) ist v3.32 Follow-Up — UI-Pattern wird identisch (eigener Sub-Tab oder Toggle-Pill 'Mono | Stereo' im Samples-Tab). Library-Layer (patchEsxBankSample) supportet bereits stereo via channels:2 + interleaved pcmData. (b) Resampling-Cost: 48kHz→44.1kHz Stereo 1s Audio ~5ms CPU in modernem Browser. Bei sehr langen Samples (>30s) kann das User-perceived freezen — TODO v3.32: WebWorker-Off-Thread-Resample. (c) decodeAudioData rejected wenn das WAV-File kompromittiert ist (z.B. 24-bit float ohne fmt-chunk) — Toast 'Fehler beim Replace' fängt das. (d) Mode-A waste persists: jeder Re-Replace eines Slots accumuliert orphan-bytes vom Vorgänger-PCM (siehe v3.30 Caveat). Mitigation v3.32+: 'Bank kompaktieren'-Action. (e) Sample-Name aus File-Basename (extension stripped, slice(0,8) auf ESX-1 8-ASCII-Limit) — User kann den Slot-Name NICHT post-decode editieren (das wäre v3.32+). (f) Drop-Target ist exakt der <li>, nicht der button — User muss auf die Zeile droppen, nicht in den Whitespace dazwischen. (g) Keine Auto-Slice-Onset-Detection auf den dropped WAV — der appended Slot hat KEINE slice-data, hardware-spielt das WAV als 1-Shot ab.",
        "package.json + agents/INDEX.js version 3.30.0 → 3.31.0."
      ],
      next: [
        "TASK-v3.31-FOLLOWUP-STEREO-SAMPLES (v3.32): Stereo-Sample-Tab analog Mono. Layout-Decision: zweiter Sub-Tab 'Samples Stereo' im Sub-Tab-Toggle ODER inline-Toggle 'Mono | Stereo' im Samples-Tab. patchEsxBankSample supportet bereits channels:2 mit interleaved pcmData. UI braucht channels:2 im Decoder-Workflow (KEIN downmix) + 128-Slot-Liste.",
        "TASK-v3.31-FOLLOWUP-WAVEFORM-PREVIEW (v3.32): Slot-Detail rechts zeigt WaveformSliceCanvas (existing v3.8) mit dem dropped/replaced PCM. Lets user verify before save. Optional: Trim-Handles links/rechts.",
        "TASK-v3.31-FOLLOWUP-COMPACT-ACTION (v3.32): 'Bank kompaktieren'-Button im Footer. Rebuild Sample-Section from scratch (eliminiert orphans, reduziert Bank-Size). Mode-B Rebuild als opt-in (NICHT default — non bit-exact).",
        "TASK-v3.31-FOLLOWUP-NAME-EDIT-INLINE (v3.32): Slot-Detail Name-Feld editable, patchEsxBankSampleHeader (nur 8B-Name in-place) ohne PCM-Replace. Schneller als full Replace bei kleinen Edits.",
        "TASK-v3.31-FOLLOWUP-WORKER-RESAMPLE (v3.32): polyPhaseResample in WebWorker für lange Samples (>30s) damit Main-Thread nicht freezed.",
        "TASK-v3.31-FOLLOWUP-SAMPLE-AUDITION (v3.32): 'Play'-Button im Slot-Detail spielt das (replaced ODER existing) PCM ab via Web-Audio (existing playSliceWithContext). Lets user pre-listen vor Save."
      ],
      changed: [
        "client/src/utils/korg/esxBankEditorState.ts (ERWEITERT: +EsxSampleSlotRow + EsxSamplePatchEntry types, +buildEsxSampleSlotOverview + stageEsxSamplePatch + unstageEsxSamplePatch + hasPendingEsxSamplePatches + countPendingEsxSamplePatches + commitEsxSamplePatches + commitEsxPatchesAll + filterEsxSampleRows + formatSampleLength)",
        "client/src/components/KorgBank/KorgBankEditor.tsx (ERWEITERT: +Sub-Tab-Toggle Patterns|Samples + renderEsxSamplesBody + handleEsxSampleReplaceClick + decodeWavForEsxSample + replaceEsxSampleSlot + handleEsxSampleRevertSlot + Drag-Drop-Handlers + neue State-Felder + Reset-Logic für ESX-Sample-State + handleEsxSaveBank nutzt commitEsxPatchesAll)",
        "tests/features/korg-esx-editor-sample.test.ts (NEU, 17 Tests in 5 describes: buildEsxSampleSlotOverview + stage/unstageEsxSamplePatch + commitEsxSamplePatches + commitEsxPatchesAll + filterEsxSampleRows + formatSampleLength)",
        "package.json (3.30.0 → 3.31.0)",
        "agents/INDEX.js (version 3.30.0 → 3.31.0 + workLog v3.31.0 entry + neue files-Einträge)"
      ]
    },
    {
      agent:     "backend",
      timestamp: "2026-05-18T15:45:00.000Z",
        ...
        "v3.30.0: ESX-1 Sample-Slot Replace in existing Bank (Bit-Exact für Rest) — Library-Layer. Pure-TS Pattern symmetrisch zur v3.28 patchEsxBankPattern, jetzt aber für PCM-Samples (40B mono / 44B stereo Header + variabel langes PCM). Mirror-Pattern: lade BOTTROP.ESX → ersetze Sample-Slot N → save bit-exact für andere Samples + alle Patterns.",
        "COMPACT-MODE-A (Append + Header-Update) implementiert (statt Mode B Rebuild). Rationale: simpler, andere Samples bleiben PHYSISCH bit-exact (gleicher Byte-Offset im File, gleiche Bytes), die Header der anderen Slots zeigen unveraendert auf die alten Regionen. Cost: alte PCM-Region des replaced Slot wird Orphan (waste = alte sample-length Bytes). Bank-Size waechst um sizeof(new PCM). Pattern-Region, Song-Region und alle Globals bleiben bit-exact (0..0x130000 + 0x130000..0x1B0000).",
        "NEU client/src/utils/korg/esxSamplePatcher.ts (~370 LOC, isomorph, KEINE Electron/DOM/AudioEngine-Deps). Public API: patchEsxBankSample(bankBuffer, patch: EsxSamplePatchInput) → ArrayBuffer (NEUER Buffer, Input wird NIE mutiert). EsxSamplePatchInput: {index 0..255 mono / 0..127 stereo, channels: 1|2, pcmData: Float32Array (mono = flach, stereo = interleaved L,R,L,R,…), sampleRate, name?: string max 8 ASCII, level?: 0..127}. Workflow (Mode A): (1) validateBankBufferForSample (magic + 0x1B0000 second-magic) → (2) validateSamplePatch (Range-Bounds, byte-Alignment, sampleRate finite > 0) → (3) float32ToBe16Pcm (Float32 [-1,+1] → BE i16, NaN/Inf-defensive → 0, clip > 1) → (4) Read counters@0x1B0020/0x1B0024 + currentOffset@0x1B0028 → (5) safeCurrent = (currentOffset==SENTINEL || > file-end) ? file-end-relative-to-0x250000 : currentOffset → (6) appendRel = max(safeCurrent, fileEndPcmRel) → (7) Cumulative-Cap-Check vs ESX1_MAX_SAMPLE_MEM_IN_BYTES (24MB) → (8) Grow Buffer auf appendAbs+newPcmBytes → (9) outBytes.set(inputBytes, 0) bit-exact copy → (10) Schreibe new PCM bytes: mono einfach @appendAbs, stereo L/R contiguous (split interleaved → 2 channel-Buffers @ appendAbs / appendAbs+channelBytes, ESX-1-Spec hat off1Start+off1End fuer L, off2Start+off2End fuer R) → (11) Update Header (mono 40B: name@0..7, off1Start@+8, off1End@+12 BE u32, frames@+20, loopStart@+24=0, sampleRate@+28 BE u32, sampleTune@+32=0, playLevel@+34 / stereo 44B: +0..7 name, +8..23 4 offsets, +24..31 frames, +32 sr, +36 tune, +38 level) → (12) wasEmpty? Counter inkrementieren (mono+1 oder stereo+1, geklemmt auf Max-Slots) → (13) currentOffset@0x1B0028 ← projectedCurrent. Public helpers: encodeEsxName (8 ASCII space-padded, non-printable→'?'), float32ToBe16Pcm (BE clipping + NaN→0), isSlotEmpty (off1Start@+8 === 0xFFFFFFFF sentinel), getEsxMonoHeaderOffset / getEsxStereoHeaderOffset (Index→absolute Offset Helpers). EsxSamplePatchError class.",
        "BIT-EXACT-REGIONEN garantiert (via FNV-1a Tests verifiziert): (a) Magic Header [0..0x20), (b) Global Parameters [0x20..0x200), (c) Pattern Region [0x200..0x130000) — alle 256 Pattern-Slots × 4280B unveraendert, (d) Song Region [0x130000..0x1B0000), (e) Sample-Section Magic + Counter-Headers an unchanged Slots + alle PCM-Regions anderer (= nicht gepatchter) Slots. Was sich AENDERT: Counter@0x1B0020/0x1B0024 (bei prev-empty Slot +1), currentOffset@0x1B0028, das eine gepatchte Header-Tabellen-Zelle, der appended PCM-Bereich am Ende des Files.",
        "TESTS NEU tests/features/korg-esx-sample-patch.test.ts (31 Tests in 7 describes, env:node, kein jsdom): (1) Validators 4× — synthetic-bank valid + KORG-magic corrupt + Sample-Section-Magic corrupt + buffer-too-small. (2) Helpers 4× — encodeEsxName (pad/truncate/non-printable) + float32ToBe16Pcm (BE bytes + clip + NaN→0 + out-of-range>1/<-1) + isSlotEmpty (mono+stereo). (3) Mono-Patch 4× — appends PCM + updates header (name/sampleRate/playLevel/offsets), increments numMono-Counter wenn empty, behaelt Counter unveraendert wenn Slot filled, updates currentOffset zum projected end. (4) Bit-Exact-Preservation via FNV-1a 6× — pattern region 0x0200..0x130000 + song region + global + magic header + andere sample-headers + andere PCM-Bytes (prefilled slot 3 unchanged bei Patch slot 25). (5) Validation-Errors 8× — out-of-range mono-index, out-of-range stereo-index, invalid channels (3), empty PCM, oversized PCM > 10MB, stereo PCM odd length, invalid sampleRate (0, NaN), corrupt bank-buffer. (6) Round-Trip via parseEsxBank 3× — patch mono sine 220Hz → parsed monoSamples[0] hat name='SINE220' + frames=512 + sampleRate=44100 + PCM mit int16-Quantisation-Error < 0.001, prefilled OLD-slot bleibt nach Patch OTHER slot bit-identisch via PCM-Vergleich Frame-by-Frame, stereo-patch decodet als 2-channel interleaved (L=110Hz, R=220Hz erste 2 Werte assertion). (7) Header-Offset-Helpers 2× — getEsxMonoHeaderOffset/getEsxStereoHeaderOffset Layout-Check + throws.",
        "TEST-RESULTAT: pnpm check clean (3.29.0 → 3.30.0). pnpm test FULL: 182 files / 4210 passed + 16 skipped. tests/features/korg-esx-sample-patch.test.ts: 31/31 passed. KEINE Regression an existing tests (alle 10 KORG-Test-Files inklusive v3.28 korg-esx-bank-patch.test.ts + v3.27 korg-esx-pattern-write.test.ts + v3.5/v3.20/v3.23 korg-esx-parser.test.ts + v3.20 korg-esx-patterns.test.ts gruen).",
        "PRO-FEATURE-GATE: Reuse PRO_FEATURE_KORG_BANK_WRITE (v3.4 — bereits fuer E2 Sample-Bank + v3.28 ESX-Pattern-Library + v3.29 ESX-Pattern-UI). KEIN neuer Const benoetigt — selbe 'KORG Hardware-Bank Write' Capability. UI-Path in v3.31 wird die Gate beim Replace-Klick anwenden.",
        "SCOPE-DECISION OPTION-A (Library + Tests only, kein UI in v3.30): Analog v3.28 Library-Pattern. v3.31 implementiert UI-Erweiterung im KorgBankEditor (Sub-Tab 'Patterns' vs 'Samples' im ESX-Mode + Sample-Slot-Liste mit Replace-Button + File-Picker → audioContext.decodeAudioData → patchEsxBankSample). v3.30 hat NUR Library + 31 Tests. Reasoning: Library + Validators komplett ausreichend fuer Round-Trip-Verifikation, UI ist orthogonal und kann unabhaengig iteriert werden.",
        "CAVEATS: (a) Mode-A waste: alte PCM-Region des replaced Slot wird Orphan-Bytes — bei mehreren Replace-Operations pro Slot kann das File aufblähen. Mitigation: User kann jederzeit eine fresh Bank exportieren wenn das stört (v3.31+ Compact-Aktion). (b) Header schreibt sampleTune=0 (kein Tune-Field im Input) — Reader ignoriert das Feld weitgehend, ESX-1 Hardware liest es als signed centi-tone. (c) Sample-rate wird unveraendert in Header geschrieben — KEIN Auto-Resample auf Hardware-Rate (44100 oder 32000). Caller muss VOR patchEsxBankSample selbst resampeln (existing audioProcessor.ts.polyPhaseResample). (d) Mode A wenn currentOffset (free-pointer) verlogen ist (z.B. wenn 3rd-party Tool ihn nicht updated): wir fall back auf file-end-relative-to-0x250000 (max(currentOffset, fileEnd) als appendRel). Sicher gegen overlap mit alten PCM, aber kann zu inflated Bank-Size fuehren wenn currentOffset deutlich < file-end. (e) Stereo-Spec: ESX-1 hat L+R contiguous, NICHT interleaved im PCM-Bereich. Wir splitten interleaved Float32-Input → 2 channel-Buffers korrekt. (f) Tests laufen synthetic (echte BOTTROP.ESX-Fixture nicht im Repo, GitIgnored). Real-File-Verifikation = v3.31+ optional via env-var test:real-files=1.",
        "package.json + agents/INDEX.js version 3.29.0 → 3.30.0."
      ],
      next: [
        "TASK-v3.30-UI-MODAL (v3.31): KorgBankEditor.tsx → Sub-Tab 'Patterns' vs 'Samples' innerhalb des ESX-Modes. Sample-Liste mit Name + Length-Frames + Channels + Sample-Rate Spalten. Per Slot: 'Replace with WAV'-Button → File-Picker → audioContext.decodeAudioData → polyPhaseResample auf 44100 (per audioProcessor.ts.convertToE2sSpec, target 44100 fuer ESX-1) → patchEsxBankSample. Save reuses esx:save-bank-as IPC (v3.29). Pro-Gate via PRO_FEATURE_KORG_BANK_WRITE. ProLockBadge im Sub-Tab.",
        "TASK-v3.30-COMPACT-ACTION (v3.31+): 'Bank kompaktieren'-Button. Reads alle nicht-empty Slots → rebuilds Sample-Section from scratch (alle PCM-Bytes nacheinander, alle Offsets re-computed) → schreibt currentOffset = realer end-of-pcm. Eliminiert orphans + reduziert Bank-Size. Mode-B Rebuild als opt-in (NICHT default — non bit-exact).",
        "TASK-v3.30-MULTI-PATCH (v3.31+): patchEsxBankSamples(bank, patches[]) für Bulk-Replace in 1 Pass. Bei Multi-Patch koennen Patches gegeneinander stack-en (Slot A append, dann Slot B append nach Slot A's neuer PCM). Atomic 2-Phase analog v3.28 patchEsxBankPatterns.",
        "TASK-v3.30-REAL-FILE-FIXTURE: ESX-Bank-Fixture mit ein paar User-Slots in tests/fixtures/esx/ adden (gitignored). Round-Trip: load BOTTROP.ESX → patch slot N → re-parse → byte-Diff < (new PCM-Bytes + ein Header). Aktuell sind alle Tests synthetic.",
        "TASK-v3.30-AUDIO-INPUT (v3.31): Direkt aus Synthstudio Sample-Manager (useSampleStore) ein .esx slot ueberschreiben — Sample.audioBuffer.getChannelData(0..1) → convertToE2sSpec → patchEsxBankSample. Skipt manuellen File-Picker."
      ],
      changed: [
        "client/src/utils/korg/esxSamplePatcher.ts (NEU, ~370 LOC: patchEsxBankSample + validateBankBufferForSample + encodeEsxName + float32ToBe16Pcm + isSlotEmpty + getEsxMonoHeaderOffset + getEsxStereoHeaderOffset + EsxSamplePatchError + EsxSamplePatchInput type. Mode-A Append+Header-Update für mono UND stereo)",
        "tests/features/korg-esx-sample-patch.test.ts (NEU, 31 Tests in 7 describes: Validators + Helpers + Mono-Patch + Bit-Exact-Preservation via FNV-1a + Validation-Errors + Round-Trip via parseEsxBank + Header-Offset-Helpers)",
        "package.json (3.29.0 → 3.30.0)",
        "agents/INDEX.js (version 3.29.0 → 3.30.0 + workLog v3.30.0 entry + 2 neue files-Eintraege)"
      ]
    },
    {
      agent:     "frontend",
      timestamp: "2026-05-18T15:30:00.000Z",
        ...
        "v3.29.0: ESX-1 Bank-Editor UI — wires v3.27 (esxPatternBuilder) + v3.28 (esxBankPatcher) zur User-Action im KorgBankEditor. Architektur-Entscheidung: Option A — dritter Tab 'ESX Patterns' im bestehenden KorgBankEditor.tsx (statt separates Modal), weniger Code-Duplizierung, gleiche Open-/Save-Patterns wie v3.7 E2-Edit-Mode. Workflow: 'ESX Patterns'-Tab → '📂 ESX-Bank öffnen' → parseEsxBank → 256 Slot-Liste mit Search/Init-Filter → 'Mit aktuellem Pattern ersetzen' pro Slot → 'Als .esx speichern' (Pre-Save confirm-Dialog).",
        "NEU client/src/utils/korg/esxBankEditorState.ts (~165 LOC, isomorph, KEINE Electron/DOM/AudioEngine-Deps). Pure-TS Editor-State-Model: buildEsxSlotOverview(bank, totalSlots=256) → 256 dense EsxSlotRow{index, empty, name, bpm, stepLength} (parser skipt empties — Overview synthesisiert Placeholders), stageEsxPatch(map, index, block 4280B) → new Map (immutable + throws auf invalid index oder block-size), unstageEsxPatch(map, index) → new Map (returns SAME ref bei unknown slot — kein allocation), commitEsxPatches(bankBuffer, pending) → ArrayBuffer (sortiert keys ascending fuer deterministic write-order, delegiert an v3.28 patchEsxBankPatterns), filterEsxRows(rows, query, hideInit, hideEmpty) → Subset (Suche per Name-Substring case-insensitive ODER Index '5'/'005', isInitPatternName 'init pattern'/'init pat'/'init' case-insens trimmed). hasPendingEsxPatches + countPendingEsxPatches helpers. EsxSlotRow + EsxEditorState types public.",
        "ERWEITERT client/src/components/KorgBank/KorgBankEditor.tsx: dritter Mode 'esx' im EditorMode-Union (zu 'new'/'edit'). Neue Props: getActiveSynthPattern?: () => SynthstudioPatternLike|null (Closure vom App-Root liest dm.getActivePattern() — Editor importiert dm NICHT direkt), globalBpm?: number (Fallback wenn pattern.bpm===null). tryChangeMode handelt 3 outgoing-modes mit confirm-Dialog bei dirty-state (newSlots.length>0 / hasUnsavedChanges(openedSlots) / hasPendingEsxPatches). loadBankFromFile filename-Sniff: .esx → loadEsxBankFromFile (parseEsxBank + buildEsxSlotOverview) → setMode('esx'). UI: 'ESX Patterns'-Tab im Header-Mode-Toggle (zusaetzlich zu 'Neue Bank'/'Bank bearbeiten'). Body: leeres Open-Prompt (📂-Button) ODER 2-Spalten-Layout: links 2/5 Slot-Liste mit Filter-Inputs (Suche + Init-Hide + Empty-Hide toggles) + rote-Dot dirty-Indicator pro gepatchtem Slot, rechts Slot-Detail (Name/BPM/Step-Length read-only + 'Mit aktuellem Pattern ersetzen'-Button + Revert-Button wenn dirty + Filename-Input). Pre-Save confirm-Dialog ('X Slot(s) wurden geändert. Alle anderen Slots/Sample-Daten/Song-Daten bleiben bit-exakt — Bank speichern?'). Save: commitEsxPatches → electron.saveEsxBankAs (Electron) ODER Blob-Download. Reset-on-Close nimmt ESX-State (Buffer/Rows/Pending/Selected/Search) mit. data-testids alle dokumentiert (esx-mode-esx, -esx-open, -esx-slot-<i>, -esx-replace, -esx-dirty-<i>, -esx-save, etc.).",
        "IPC-CHANNEL 'esx:save-bank-as' in electron/main.ts: validateEsxBankSaveFilename → Normalize ArrayBuffer/Uint8Array/number[]-input → 16B-prefix → validateEsxBankBuffer (Size+Magic-Check) → dialog.showSaveDialog (filters [{name:'KORG ESX-1 Bank', extensions:['esx']}]) → final path.extname-Check ('.esx') → fs.writeFile. Path-Traversal-Vektor eliminiert (Pfad aus Dialog, NICHT vom Renderer). +'esx:get-bank-save-cap' returnt ESX_BANK_SAVE_MAX_BYTES=64MB. Companion electron/preload.ts saveEsxBankAs Bridge (ipcRenderer.invoke + Array.from-Transport) + electron/types.d.ts API-Decl + electron/useElectron.ts Stub (Browser-Fallback success:false → Editor faellt automatisch auf Blob-Download zurueck via electron.isElectron-Branch). Bestehende v3.4 korg:save-bank-as (.all) + v3.26 electribe:save-pattern (.e2spat) Channels komplett unveraendert.",
        "INTEGRATION client/src/App.tsx: KorgBankEditor wird mit getActiveSynthPattern={() => dmRef.current.getActivePattern() ?? null} + globalBpm={project.bpm} aufgerufen. korg:bank:open-Handler routet jetzt .esx-Drops bei offenem Editor auch in den Editor (vorher nur .all) — User kann .esx-Drag-Drop direkt in den ESX-Mode landen lassen.",
        "TESTS NEU tests/features/korg-esx-editor.test.ts (19 Tests in 6 describes, env:node, kein jsdom): (1) buildEsxSlotOverview 2× — 256 dense rows mit 3 seed patterns @ slots 0/1/5 (slots 2/3 empty placeholders, slot 5 SEED5 BPM 175) + all-empty rows bei leerer Bank. (2) stage/unstageEsxPatch 5× — stages immutably + counts pending, replaces existing patch (last-write-wins) beim 2. stage same slot, unstage returns SAME ref bei unknown slot (kein Allocation), throws auf invalid index (-1/256/1.5), throws bei 4279/4281-byte blocks. (3) commitEsxPatches Bit-Exact 3× — Single-Patch slot 5 → alle 255 anderen Slots FNV-1a-identical + Round-Trip parseEsxBank zeigt PATCH5 BPM 175, Multi-Patch slots 0/100/255 in Reverse-Stage-Order → deterministic ascending write-order verifiziert via parse, Empty pending → fresh-copy with identical bytes (committed !== input ref). (4) isInitPatternName 2× — case-insens. trimmed matches + ignores unrelated. (5) filterEsxRows 4× — hideEmpty + hideInit + name-substring + Index-match ('5'/'005'). (6) End-to-End Editor→Save flow 3× — convert→build→stage→commit→IPC-validators full accept (Filename + 16B-prefix on committed buffer), IPC filename validator rejects unsafe variants (my.all, ../etc/passwd.esx, NUL, slash, empty), parseEsxPattern of patched slot at #42 returns identical name='ROUND' + BPM=175 + stepLength=16.",
        "TEST-RESULTAT: pnpm check clean (3.28.0 → 3.29.0 version bump). pnpm test korg-esx-editor → 19 passed. pnpm test korg-esx-bank-patch + korg-esx-pattern-write + korg-esx-parser + korg-esx-patterns + security-ipc → 177 passed + 1 skipped (BOTTROP real-file fixture nicht im Repo) — KEINE Regression an existing tests.",
        "PRO-FEATURE-GATE: Reuse PRO_FEATURE_KORG_BANK_WRITE (v3.4 — bereits fuer E2 + v3.28 ESX-Library) im 'esx:save-bank-as'-UI-Path. Klick auf 'ESX-Bank öffnen' / 'Mit aktuellem Pattern ersetzen' / 'Als .esx speichern' geht durch requireProFeature. ProLockBadge im Header + Save-Button rendered. KEIN neuer Pro-Feature-Const.",
        "CAVEATS: (a) UI ist read-only fuer existing pattern-metadata — User kann nur den GANZEN Slot durch das aktive Synthstudio-Pattern ersetzen, NICHT Per-Slot-Felder editieren (Name/BPM kommen aus dem replacenden Pattern). Inline Name/BPM-Editing wäre v3.30+. (b) Slot-Highlight nutzt rote-Dot (text-accent-danger) statt v3.7 Sekundär-Accent — bewusste Unterscheidung 'destructive replace' vs E2-edit-Slot-modify. (c) Filter ist client-only — kein Persist in Settings (jeder Bank-Open frisch). (d) Es gibt keinen 'Diff/Vorschau'-Modus pre-commit — nur das confirm()-Dialog. (e) parseEsxBank ist O(n) und kann bei korrupten Files Warnings produzieren, die in den Toast geloggt werden aber das Laden nicht blocken. (f) Empty-Filter ist by-default ON, Init-Filter by-default ON (most users haben volle Init-Pattern-Banks). (g) Save schreibt KEINE Sample-Daten — wenn der User ein Synthstudio-Pattern mit sample-references replaced, sind die referenzierten ESX-1 sample-slots NICHT mit-gepatched (das wäre v3.30+ Full-Bank-Write).",
        "package.json + agents/INDEX.js version 3.28.0 → 3.29.0."
      ],
      next: [
        "TASK-v3.29-FOLLOWUP-INLINE-NAME-EDIT (v3.30): ESX-Slot-Detail soll auch ohne Synthstudio-Pattern-Replace direktes Name/BPM-Editing erlauben — patche nur 8B-Name + BPM-u16-BE im bestehenden Slot statt full 4280B-Block-Replace. Braucht Per-Slot-Field-Patcher in esxBankPatcher.ts.",
        "TASK-v3.29-FOLLOWUP-PREVIEW (v3.30): Vor 'Mit aktuellem Pattern ersetzen' eine kleine Vorschau zeigen — Pattern-Step-Grid mit Trigger-Punkten + BPM/Steps. Statt confirm()-Dialog ein modaler 2-Step-Wizard.",
        "TASK-v3.29-FOLLOWUP-REAL-FILE-FIXTURE: ESX-Bank-Fixture in tests/fixtures/esx/ adden (gitignored, dev-only). Optional via env-var test:real-files=1 gating. Aktuell sind alle Tests synthetic.",
        "TASK-v3.30-FULL-BANK-WRITE: buildEsxBank(input) für komplette ~25MB .esx-Datei from-scratch (Header, Sample-Headers, 256 patterns von Grund auf, Slice-Data, PCM). Mirror v3.6 e2sBankBuilder.fullBuild-Branch.",
        "TASK-v3.29-FOLLOWUP-DRAG-DROP-IN-EDITOR: Aktuell wird .esx-Drop nur in den BEREITS offenen Editor geroutet. Drop bei geschlossenem Editor öffnet noch den Read-Only KorgBankModal (v3.3). Optional: auch im closed-state direkt in den Editor wenn User Pro-Lizenz hat."
      ],
      changed: [
        "client/src/utils/korg/esxBankEditorState.ts (NEU, ~165 LOC: buildEsxSlotOverview + stageEsxPatch + unstageEsxPatch + commitEsxPatches + filterEsxRows + isInitPatternName + countPendingEsxPatches + hasPendingEsxPatches + EsxSlotRow/EsxEditorState types)",
        "client/src/components/KorgBank/KorgBankEditor.tsx (+~400 LOC: ESX-Mode + 3. Tab 'ESX Patterns' + Open-Flow + 256-Slot-Liste mit Filter + Replace-Action + Save-Flow + Browser-Fallback. Props erweitert um getActiveSynthPattern + globalBpm)",
        "client/src/App.tsx (KorgBankEditor wird mit getActiveSynthPattern + globalBpm aufgerufen, korg:bank:open Drop-Handler routet jetzt .esx auch in den offenen Editor)",
        "electron/main.ts (+ipcMain.handle 'esx:save-bank-as' + 'esx:get-bank-save-cap' + Import von validateEsxBankSaveFilename/validateEsxBankBuffer/ESX_BANK_SAVE_MAX_BYTES)",
        "electron/preload.ts (+saveEsxBankAs + getEsxBankSaveCap contextBridge-methods)",
        "electron/types.d.ts (+saveEsxBankAs + getEsxBankSaveCap im ElectronAPI-Interface)",
        "electron/useElectron.ts (+saveEsxBankAs Browser-Fallback-Stub + getEsxBankSaveCap async-mirror + Electron-Mode-Delegation api ?? browserAPI)",
        "tests/features/korg-esx-editor.test.ts (NEU, 19 Tests in 6 describes: buildEsxSlotOverview + stage/unstageEsxPatch + commitEsxPatches bit-exact + isInitPatternName + filterEsxRows + End-to-End Editor→Save IPC-Validator-Flow)",
        "package.json (3.28.0 → 3.29.0)",
        "agents/INDEX.js (version + 7 neue files-Eintraege + workLog v3.29.0 entry)"
      ]
    },
    {
      agent:     "backend",
      timestamp: "2026-05-18T15:00:00.000Z",
        ...
        "v3.28.0: ESX-1 Bank Pattern-Patch — Replace Pattern-Slot in existing .esx (Mirror v3.6 Raw-RIFF-Pattern). Pragmatischer Slot-Replace-Workflow analog zu e2sBankBuilder.preserveRawRiff: lade ~25MB .esx → ueberschreibe ein oder mehrere 4280-Byte Pattern-Slots bit-exact → schreibe zurueck. Vermeidet Full-Bank-From-Scratch (256 patterns + 384 samples + 24MB PCM + slice tables + song data + global section).",
        "NEU client/src/utils/korg/esxBankPatcher.ts (~260 LOC, isomorph, pure-TS, KEINE Electron/DOM-Deps). Public API: patchEsxBankPattern(bankBuffer, patternIndex 0..255, newPatternData 4280B) → ArrayBuffer (same size as input) + patchEsxBankPatterns(bankBuffer, patches: Array<{index, data}>) fuer Bulk-Replace in einem Pass. Defensive Validierung VOR jedem Write: KORG-Magic @0x00 + ESX\\0-Submagic @0x08 + Size-Range [ESX1_SIZE_FILE_MIN..ESX_FILE_MAX_BYTES] + patternIndex Integer 0..255 + newPatternData byteLength === ESX1_CHUNKSIZE_PATTERN (4280) + Bank-Buffer groß genug für Ziel-Slot + Bulk-Patch dedupliziert via Set<index> (Duplicate-Throw). Atomic-Semantik: all-or-nothing — bei einem invalid patch wird gar nichts geschrieben. Output ist NEUER Buffer (Input wird NIE mutiert). Public helpers exportiert: validateBankBuffer (throws), looksLikeEsxBank (boolean), getEsxPatternSlotOffset(index) → byte-offset 0x0200 + i*4280, EsxBankPatchError, EsxBankPatch type.",
        "BIT-EXACT-VERIFY HELPER: fnv1aHash32(bytes, start?, end?) → 32-bit FNV-1a hash für efficient byte-range integrity checks. Used in tests to prove preservation of [0x00..0x20), [0x20..0x200) (global params), [0x130000..0x1B0000) (song), [0x1B0000..EOF) (sample section + PCM), und alle 255 unverbatim Pattern-Slots beim Single-Patch.",
        "ELECTRON-IPC ERWEITERT electron/ipcValidators.ts: NEU validateEsxBankSaveFilename (whitelist /^[A-Za-z0-9._-]+\\.esx$/ + NUL/slash/dotdot-defense + max 120 chars) + validateEsxBankBuffer (Min ESX_BANK_SAVE_MIN_BYTES=0x00250010 + Max 64MB + KORG@0x00 + ESX\\0@0x08 prefix-check). Separater Pfad zur bestehenden .all (E2S) Bank-Save-Validation, damit Endungs-Pflicht pro Format strikt enforced bleibt. KORG_BANK_SAVE_MAX_BYTES=256MB-Cap auch fuer ESX-Path passend (ESX-Files sind 24-28MB Real-Range, 64MB-Cap ist 2x headroom). Save-IPC-Channel selbst NICHT in v3.28 (Library + Validators only, kein UI — kommt v3.29).",
        "TESTS NEU tests/features/korg-esx-bank-patch.test.ts (29 Tests in 8 describes, env:node): (1) Validators 4× (synthetic-bank valid, KORG-magic corrupt throw, ESX-submagic corrupt throw, buffer-too-small throw). (2) getEsxPatternSlotOffset 3× (slot 0 = 0x0200, slot 5 = 0x0200+5×4280, out-of-range throws fuer -1/256/1.5). (3) Single-Slot-Patch 4× (overwrites exact 4280B at right offset, byteLength preserved, accepts ArrayBuffer+Uint8Array beide, input NICHT mutiert). (4) Bit-Exact-Preservation via FNV-1a 5× (255 andere slots unchanged + global region 0x20..0x200 unchanged + magic 0..0x20 unchanged + song 0x130000..0x1B0000 unchanged + sample section 0x1B0000..EOF unchanged). (5) Validation-Errors 5× (patternIndex out-of-range -1/256/1.5, wrong patch-data-size 4279/4281, invalid magic, duplicate bulk indices, non-array patches). (6) Bulk-Patch 2× (3 patches at slots 0/100/255 each correctly applied + 253 untouched slots bit-exact, empty patches → identical-bytes copy NEW buffer). (7) CRITICAL Round-Trip via parser 2× (parseEsxBank seeded → patch slot 5 → re-parse: slot 5 = new name+bpm, slots 0/1/2/3/4/6/7 byte-identical via FNV; parseEsxPattern of patched slot returns exact name+BPM+stepLength). (8) FNV-1a hash helper sanity 3× (identical bytes same hash, single-byte change different hash, start/end slice arguments honored). +1 conditional skip (real-file BOTTROP.ESX test — fs.existsSync gate, fixture nicht im Repo).",
        "TEST-RESULTAT: pnpm check clean (3.27.0 → 3.28.0 version bump). pnpm test tests/features/korg-* alle 10 KORG-Test-Files gruen: 294 passed + 1 skipped (real-file). pnpm test tests/features/security-ipc.test.ts: 42 passed — KEINE Regression an existing IPC-Validators. Insgesamt v3.28 +28 neue Bank-Patcher-Tests + neue ESX-Bank-Save-Validators (4 weitere Tests integrierbar in security-ipc.test.ts wenn benoetigt).",
        "ROUND-TRIP-VERIFIED: parseEsxBank(syntheticBank seeded with 8 SEED-patterns) → patchEsxBankPattern(slot 5, PATCH5-block) → parseEsxBank(patched) liefert: slot 5 name='PATCH5' BPM=175 (war SEED5 BPM=125), slots 0..4 + 6..7 byte-identical zu pre-patch (FNV-1a per-slot hashes match). Reader confirmiert dass der neue Slot-Inhalt korrekt decodiert wird + alle anderen Slots unveraendert sind.",
        "BIT-EXACT-VERIFICATION-STRATEGY: FNV-1a 32-bit hash via Math.imul für byte-range integrity. Pro Region (header / globals / pre-patch slots / patched slot / post-patch slots / song / sample section) wird hash(input) vs hash(output) verglichen. Schneller als byte-loop bei 25MB Files (FNV-1a ist O(n) mit ~2 CPU-cycles/byte via Math.imul). Plus byte-loop spot-checks fuer kritische Felder.",
        "PRO-FEATURE-GATE: Reuse PRO_FEATURE_KORG_BANK_WRITE (v3.4) — gilt jetzt fuer E2-Sample-Bank-Write UND ESX-Bank-Pattern-Patch. KEIN neuer Pro-Feature-Const benoetigt (selbe Use-Cases-Category 'KORG Hardware-Bank Write'). v3.28 Library-only — UI-Gate kommt in v3.29 mit dem Pattern-Patch-Modal.",
        "SCOPE-DECISION OPTION-A: Library + Validators + Tests only. v3.29 implementiert separately den UI-Pattern-Tab in KorgBankEditor + IPC-Channel 'esx:save-bank-as' (analog 'korg:save-bank-as' fuer .all). Empfehlung im Task: 'Option A' wurde umgesetzt — kein DrumMachine-Toolbar-Button in v3.28.",
        "CAVEATS: (a) Real-File-Test gegen BOTTROP.ESX ist conditional skip (fs.existsSync(__dirname + '/../../fixtures/esx/BOTTROP.ESX')). Repo enthält keine .esx Fixtures (24MB+ pro File), Tests laufen synthetic. (b) Patcher validiert NICHT den Inhalt der newPatternData — Caller muss buildEsxPatternBlock(...) oder vergleichbare 4280B-Producer nutzen (z.B. eine subarray aus einer anderen Bank). looksLikeEsxPatternBlock im esxPatternBuilder.ts ist als opt-in available. (c) Patcher schreibt KEINE Sample-Headers, Song-Daten, Globals oder Slice-Data — diese bleiben bit-exact erhalten. Wer eine komplette Bank schreiben will, braucht Full-Bank-Write (v3.30+). (d) Bulk-Patch ist atomar via 2-Phase: Validate-All + Then-Write-All. Es gibt KEINE partielle Write-Semantik. (e) Empty patches array → identical-bytes copy in NEW buffer (defensive — Caller kann original nicht mutieren via Return-Value).",
        "package.json + agents/INDEX.js version 3.27.0 → 3.28.0."
      ],
      next: [
        "TASK-v3.28-UI-MODAL (v3.29): KorgBankEditor.tsx → neuer Pattern-Tab mit 256 Slot-Grid + Per-Slot-Edit-Modal (Name/BPM/Steps editierbar) + Save-As-Button mit Pro-Gate. Patches sammeln in {index:number, data:ArrayBuffer}[] und ein-shot via patchEsxBankPatterns commit.",
        "TASK-v3.28-IPC-CHANNEL (v3.29): electron/main.ts → ipcMain.handle('esx:save-bank-as', async (event, suggestedFilename, data) => ...) mit validateEsxBankSaveFilename + validateEsxBankBuffer Validierung VOR fs.writeFile. Companion preload.ts saveEsxBank Bridge + useElectron Hook Wrapper mit Browser-Fallback (Blob-Download).",
        "TASK-v3.28-FULL-BANK-WRITE (v3.30+): buildEsxBank(input) für komplette ~25MB .esx-Datei from-scratch. Header @ 0x0000 KORG+ESX\\0, second-magic @ 0x001B0000, sample-counters @ 0x001B0020, 256 mono + 128 stereo headers, 256 pattern-blocks (jetzt nutzbar via buildEsxPatternBlock), Slice-data @ 0x001B4200, PCM @ 0x00250000.",
        "TASK-v3.28-REAL-FILE-FIXTURE: Sample-.esx als test-only-Fixture an tests/fixtures/esx/ adden (gitignored, dev-machine-only). Real-File-Test wird dann automatisch aktiv via fs.existsSync. Optional via env-var test:real-files=1 gating."
      ],
      changed: [
        "client/src/utils/korg/esxBankPatcher.ts (NEU, ~260 LOC: patchEsxBankPattern + patchEsxBankPatterns + validateBankBuffer + looksLikeEsxBank + getEsxPatternSlotOffset + fnv1aHash32 + EsxBankPatchError + EsxBankPatch type)",
        "electron/ipcValidators.ts (+validateEsxBankSaveFilename + validateEsxBankBuffer + ESX_BANK_SAVE_FILENAME_REGEX/MAX_LEN/MAX_BYTES/MIN_BYTES + EsxBankFilenameCheck + EsxBankBufferCheck types — separat von KORG_BANK_SAVE (.all) damit Endungs-Pflicht pro Format strikt enforced)",
        "tests/features/korg-esx-bank-patch.test.ts (NEU, 29 Tests in 8 describes: Validators + getEsxPatternSlotOffset + Single-Slot-Patch + Bit-Exact-Preservation via FNV-1a + Validation-Errors + Bulk-Patch + CRITICAL Round-Trip via parser + FNV-1a helper sanity + 1 conditional real-file skip)",
        "package.json (3.27.0 → 3.28.0)",
        "agents/INDEX.js (version 3.27.0 → 3.28.0 + workLog v3.28.0 entry + neue files-Eintraege fuer esxBankPatcher.ts und korg-esx-bank-patch.test.ts)"
      ]
    },
    {
      agent:     "backend",
      timestamp: "2026-05-18T14:30:00.000Z",
        ...
        "v3.27.0: ESX-1 Pattern Block Write — Mirror von v3.14/v3.20/v3.23 Read. Pure-TS Binary Builder fuer 4280-Byte ESX-1 Pattern-Blocks (NICHT komplette .esx-Bank — Full-Bank-Write ist v3.28+).",
        "NEU client/src/utils/korg/esxPatternBuilder.ts (~400 LOC, isomorph, KEINE Electron/DOM-Deps). Public API: buildEsxPatternBlock(input: EsxPatternInput) → ArrayBuffer (immer EXAKT 4280 Bytes = ESX1_CHUNKSIZE_PATTERN-Constant aus korg/constants.ts geshared). Layout-Spec mirror der v3.14/v3.20/v3.23 Read-Side: 0x000..0x007 8B Name ASCII space-padded (truncated >8 char, non-printable → '?'), 0x008..0x009 BPM × 128 BE u16 (clamp 20..300, round), 0x00D StepLength-1 (default 0x0F=16 steps, range 1..64), 0x00F Swing 0..100 best-effort, 0x018..0x163 10 Drum-Parts × 34B-Stride, 0x25C Stretch-Part (Part 10) 34B, 0x36E/0x38E/0x3AE/0x3CE 4 Short-Parts (Sample/Slice/Synth) 32B-Stride. Drum/Stretch-Part-Layout: +0..+1 sampleId BE u16 (0x8000 unassigned), +2..+3 'ff 00' invariant marker (geschrieben gegen Real-File-Verhalten), +8 pitch (signed i8 around 0x40 neutral), +9 level (u8 0..127 default 100), +10 pan (u8 0..127 default 64), +11 fxSend (u8 0..127), +18..+33 16 step-bytes. Short-Part-Layout: +0..+1 sampleId, +6 pitch, +7 level, +8 pan, +10 fxSend, +16..+31 16 step-bytes. Step-Byte: bit 0 = active (trigger), bit 4 = accent (TR-style velocity-boost). Defensive: clampInt(value, min, max, fallback) ueberall fuer BPM/level/pan/fxSend/pitch/swing/sampleId (9-bit mask). Public Sub-Exports: writeDrumPartBlock/writeShortPartBlock + encodeStepByte/encodePitchByte/encodeSampleId pure helpers + looksLikeEsxPatternBlock-Validator. Constants exported: ESX_PATTERN_BLOCK_SIZE, ESX_DRUM_PARTS_OFFSET, ESX_DRUM_PART_STRIDE, ESX_STRETCH_PART_OFFSET, ESX_SHORT_PART_OFFSETS, alle Layout-Offsets, ESX_SAMPLEID_UNASSIGNED, ESX_PITCH_NEUTRAL_RAW, ESX_STEP_TRIGGER_BIT, ESX_STEP_ACCENT_BIT.",
        "ERWEITERT client/src/utils/korg/esxPatternConvert.ts (+~170 LOC zum bestehenden v3.5 Read-Convert). NEU convertSynthstudioPatternToEsx(pattern: SynthstudioPatternLike) → EsxPatternInput. Mapping (1:1 partIndex 0..14): Synthstudio parts[0..9] → drumParts[0..9], parts[10] → stretchPart, parts[11..14] → shortParts[0..3], parts[15] → audioInPart (typically silent). volume (0..1) → level (0..127) via synthVolumeToEsxLevel (round), pan (-1..+1) → 0..127 via synthPanToEsxPan (64+p*63), velocity > 100 → accent=true (ESX_ACCENT_VELOCITY_THRESHOLD const), pitch aus erstem aktiven Step mit definiertem pitch-Feld (best-effort, ESX hat nur 1 per-Part-Pitch). Name truncated auf 8 Chars (Hardware-Spec). BPM null/undefined → 120 Fallback. Structurally-compatible SynthstudioPatternLike/SynthstudioPartLike Types ohne AudioEngine-Dependency (pure-Logik). Helpers exportiert: synthVolumeToEsxLevel, synthPanToEsxPan, synthStepToEsx, synthPartToEsxDrumPart, synthPartToEsxShortPart.",
        "SCOPE: v3.27 ist PURE-BUILDER-LIBRARY ohne UI. Kein neuer Toolbar-Button — User-Facing UI fuer ESX-Pattern-Write kommt in v3.28 (full Bank Write) oder v3.29 (Pattern-Export-Modal). Output ist 4280-Byte-Block einsetzbar an Position 0x0200 + i × 4280 einer .esx-Bank.",
        "TESTS NEU tests/features/korg-esx-pattern-write.test.ts (34 Tests in 9 describes, env:node): (1) Builder-Basics 4× (4280-Byte exact + looksLikeEsxPatternBlock-Validator + 4279-Byte rejection + 'ff 00' invariant marker presence). (2) Header-Fields 6× (Name space-padding short, truncate long >8, BPM=175 → 0x5780 BE u16, Range-Clamp [10→20, 999→300, 60.5 preserved], StepLength=16→0x0F + 32→0x1F, Swing 75 + clamp 200→100). (3) Drum-Part-Layout 5× (sampleId BE u16 + 0x8000 unassigned sentinel, pitch ±semitones around 0x40 [0→0x40, -10→0x36, +12→0x4C], level/pan/fxSend offsets +9/+10/+11, clamp out-of-range, 4-on-the-floor steps via bit 0). (4) Step-Byte 4× (encodeStepByte: inactive→0x00, active→0x01, active+accent→0x11, inactive+accent→0x00). (5) Short-Part-Layout 3× (sampleId BE u16 @0x36E, pitch/level/pan/fxSend offsets +6/+7/+8/+10, step-bytes @+16). (6) Stretch-Part 1× (34B drum-layout @0x25C verified). (7) Encoding-Helpers 2× (encodePitchByte ±range + encodeSampleId 9-bit mask + 0x8000 unassigned + negative). (8) CRITICAL Round-Trip 2× (programmiertes 3-Drum/1-Stretch/1-Short Pattern incl. accent → parseEsxPattern produces identical name/bpm/lengthSteps/swing/parts[].sampleId/pitch/level/pan/fxAmount/steps[].active/accent/velocity-boost-127 + BPM stability across [60,100,120,128,140,175,200,240]). (9) Synthstudio-Adapter 7× (synthVolumeToEsxLevel scale 0..1→0..127, synthPanToEsxPan 64+p*63, synthStepToEsx velocity-threshold-100 accent-mapping, full convertSynthstudioPatternToEsx kick-pattern + truncated-name + drumParts[10]+shortParts[4] arrays, threshold-constant=100, full round-trip Synth→Adapter→Builder→Parse fuer kick-pattern BPM 140, missing BPM fallback to 120 + empty parts padding).",
        "TEST-RESULTAT: pnpm check clean (3.27.0). pnpm test → 179 files / 4132 passed / 15 skipped (vorher v3.26 4098 → +34 neue ESX-Write-Tests). Alle existing Tests bleiben gruen — KEINE Regressionen.",
        "ROUND-TRIP-VERIFIED: build → parseEsxPattern liefert identical input fuer name (8-Char-Truncated), BPM (exact via × 128 BE u16 encoding), lengthSteps (1..64), swing (0..100), drum-parts sampleId+pitch+volume+pan+fxAmount, alle 16 step-bytes inkl. accent-Mapping (active+accent → velocity 127 / active ohne accent → 100 / inactive → 0). Short-parts (11..14) ebenso bit-genau geprueft. Reader-Side hat keine offenen Felder, die der Builder nicht kann.",
        "DEFENSIVE-DEFAULTS: (a) Audio-In Part 15 bleibt zero (in Real-Files immer empty laut v3.20 Doku). (b) Pitch der Drum-Parts via signed-i8 around 0x40 — Reader hat 'unsigned-read with signed-correction' (KASSEL.esx Werte 0x80..0xFF), wir schreiben 0x00..0x7F (Range -64..+63 semitones) was die Standard-Range ist. (c) 'ff 00' invariant marker @ part+2..+3 explizit geschrieben (Reader prueft das nicht, aber Real-Files haben es immer). (d) Short-Part Byte @+9 (often 0x7F in real files) bleibt 0 — Reader ignoriert.",
        "package.json + agents/INDEX.js version 3.26.0 → 3.27.0."
      ],
      next: [
        "TASK-v3.27-FULL-BANK-WRITE (v3.28+): buildEsxBank(input: EsxBankInput) → ArrayBuffer fuer komplette ~25MB .esx-Datei. Header @ 0x0000 'KORG'+'ESX\\0', second-magic @ 0x001B0000, sample-counters @ 0x001B0020, 256 mono-headers + 128 stereo-headers, 256 pattern-blocks @ 0x0200 (jetzt nutzbar via buildEsxPatternBlock), Slice-data @ 0x001B4200, PCM-Payload-Bereich @ 0x00250000 (alle relativ-Offsets in Sample-Headers).",
        "TASK-v3.27-MOTION-LANES: 0x16C..0x25B (Drum-Motion) + 0x27E..0x35D (Stretch+Sample-Motion) sind v3.27 zero-padded. Real-Files haben hier Lane-Daten (15 lanes × 16B Pitch-Motion + Switch-Lanes). RE-Pass benoetigt + EsxMotionInput Type.",
        "TASK-v3.27-UI-EXPORT: Pattern-Export-Modal mit Pro-Gate + Filename-Sanitization + IPC-Save-Channel 'esx:save-pattern-block'. Aktuell ist v3.27 nur Library — kein UI / IPC.",
        "TASK-v3.27-REAL-FILE-ROUNDTRIP: parseEsxBank(BOTTROP.esx) → patterns[0] → build wieder → byte-Diff < 50 Byte (Motion-Lanes + Reserve bleiben unchanged). Aktuell nur synthetic round-trips."
      ],
      changed: [
        "client/src/utils/korg/esxPatternBuilder.ts (NEU, ~400 LOC: Pure-TS 4280-Byte builder + writeDrumPartBlock + writeShortPartBlock + encodeStepByte/encodePitchByte/encodeSampleId helpers + looksLikeEsxPatternBlock-Validator + Layout-Konstanten-Re-Exports)",
        "client/src/utils/korg/esxPatternConvert.ts (+~170 LOC: NEU convertSynthstudioPatternToEsx + synthVolumeToEsxLevel + synthPanToEsxPan + synthStepToEsx + synthPartToEsxDrumPart + synthPartToEsxShortPart + SynthstudioPatternLike/PartLike pure-types + ESX_ACCENT_VELOCITY_THRESHOLD const)",
        "tests/features/korg-esx-pattern-write.test.ts (NEU, 34 Tests in 9 describes: Builder-Basics + Header-Fields + Drum-Part-Layout + Step-Byte-Bit-Encoding + Short-Part-Layout + Stretch-Part + Encoding-Helpers + CRITICAL Round-Trip + Synthstudio-Adapter mit Full-Round-Trip)",
        "package.json (3.26.0 → 3.27.0)",
        "agents/INDEX.js (version 3.26.0 → 3.27.0 + workLog v3.27.0 entry + neue files-Eintraege)"
      ]
    },
    {
      agent:     "backend",
