#!/usr/bin/env node
/**
 * Synthstudio – App-Icon Generator (CommonJS Version)
 *
...
async function generateIcon() {
  // Canvas-Modul dynamisch laden
  let canvasModule;
  try {
    canvasModule = require('canvas');
  } catch (err) {
    console.warn('[IconGen] Paket "canvas" nicht gefunden.');
    console.info('[IconGen] Installation: npm install -D canvas');
  ...
  console.info('  ICO (Windows):  npm install -D png-to-ico');
  console.info('                  npx png-to-ico assets/icon.png > assets/icon.ico');
  console.info('  ICNS (macOS):   Nutze Online-Tool: https://www.icoconverter.com/');
}
