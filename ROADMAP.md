...
| **BUG-018** | ✅ Popup-X-Button closet die komplette App (Regression in v1.28.0) | **critical** (data loss risk) | Fixed in vier Wellen. **v1.29.0**: Modul-Flag `mainWindowDestroyed` + `window-all-closed`-Handler verweigert `app.quit()` wenn Flag `false`. **v1.30.0**: nukleare Quit-Sperre via neuer `userInitiatedQuit`-Whitelist. **v1.31.0 (nach erneuter User-Bestätigung dass v1.30.0 fix nicht reicht)**: Vermutung war OS-Window-Cascade auf Windows (Schließen eines Child-Windows mit parent:mainWindow feuert ein WM_CLOSE auf den Parent). Zwei zusätzliche Hardenings: (a) `markPopupClosed()` setzt `lastPopupCloseTime`-Timestamp bei jedem Popup-Close; `mainWindow.on('close')` `preventDefault()`'t wenn der Close <300ms NACH einem Popup-Close kommt (= Cascade-Detection). (b) UX-Redesign auf User-Vorschlag: `DetachableWindowHeader` hat jetzt nur EINEN Button "📌 Anpinnen" der das Popup schließt (= Reattach via main-side `popupClosed`-Listener); kein separates ✕ mehr; Always-on-top entfernt. Reduziert die Angriffsfläche + macht den Workflow eindeutig. |
