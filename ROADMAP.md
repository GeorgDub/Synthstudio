...
| **Live Step Recording** | ✅ Umgesetzt (post-v1.30.0). Neuer `useLiveStepRecorder`-Hook in `client/src/hooks/`. Wenn `isRecording && isPlaying` aktiv, werden eingehende MIDI-Noten (via existierendem `stepinput:noteon`-CustomEvent) automatisch auf den aktuell spielenden Step (`AudioEngine.currentStep`) der aktiven Pattern gesetzt. MIDI-Note → Part-ID Mapping via `getGmDrumInfo`-Kategorie (kicks/snares/hihats/claps/toms/percussion) + case-insensitive substring-match auf part.name. Velocity-Wert wird übernommen (1-127 clamped). Overdub-Verhalten: bestehende aktive Steps bleiben erhalten. 17 neue Vitest-Tests in `tests/features/live-step-recorder.test.ts` für die deterministische Mapping-Logik (Kategorien + Edge-Cases). |
