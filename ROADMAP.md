...
| **BUG-018** | ✅ Popup-X-Button closet die komplette App (Regression in v1.28.0) | **critical** (data loss risk) | Fixed in drei Wellen. **v1.29.0**: Modul-Flag `mainWindowDestroyed` + `window-all-closed`-Handler verweigert `app.quit()` wenn Flag `false`. **v1.30.0 (nach erneuter User-Bestätigung dass v1.29.0 fix nicht reicht)**: nukleare Quit-Sperre via neuer `userInitiatedQuit`-Whitelist. Alle legitimen Quit-Pfade markieren das Flag explizit (mainWindow.on close, Tray "Beenden", Datei→Beenden mit mainWindow-Fokus, Updater quitAndInstall, Single-Instance-Lock-Fail). `before-quit`-Handler `event.preventDefault()`'t alles andere und zeigt mainWindow wieder an. Plus UX: Pin-Button-Label in `DetachableWindowHeader` umbenannt von "Pin/Pinned" zu "⬆ Top / Top ON" damit User es nicht mit "Re-Dock" verwechselt — Re-Dock geht via ✕. |
