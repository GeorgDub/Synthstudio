...
| **BUG-018 / BUG-019** | ✅ Popup-X/Anpinnen closet die komplette App (mehrere Wellen) | **critical** (data loss risk) | Fixed in fünf Wellen. **v1.29.0**: `mainWindowDestroyed`-Flag + `window-all-closed`-Refusal. **v1.30.0**: `userInitiatedQuit`-Whitelist + `before-quit`-PreventDefault für nicht-whitelist'ed Quits. **v1.31.0**: Cascade-Detection via `lastPopupCloseTime` (300ms-Fenster); `DetachableWindowHeader` Single-Button "📌 Anpinnen" Redesign. **v1.34.0 (BUG-019 follow-up)**: User-Report dass Reattach immer noch crasht. ROOT-CAUSE-FIX: `parent: mainWindow` ENTFERNT aus ALLEN 6 Popup-Window-Creations. Auf Windows kann ein WM_CLOSE von einem child mit parent-config auf den Parent cascaden — durch Entfernen der parent-Beziehung können Popups mainWindow nicht mehr aus Versehen runter ziehen. Popups sind jetzt truly independent windows. Cascade-Close MAIN→POPUPS bleibt programmatisch in `mainWindow.on('closed')`. |
| **BUG-020** | ✅ Performance Mode Popup hatte keinen Anpinnen-Button, ESC ✕ closed App | high (UX + crash trigger) | Fixed (post-v1.33.0). Ursache: `PatternLaunchPad` rendered mit `fixed inset-0 z-50`, was den `DetachableWindowHeader` im Popup komplett überdeckte. User sah nur die innere ESC ✕ Button die `closePerformanceWindow` triggerte — was vor v1.34.0 den Quit-Cascade getriggert hat. Fix: neuer `popupMode`-Prop auf `PatternLaunchPad`. Im Popup-Modus (gesetzt von `PerformancePopupApp`) rendert die Komponente `flex-1 min-h-0` relative statt `fixed inset-0` absolute → DetachableWindowHeader bleibt oben sichtbar. Inline-Verhalten (Vollbild-Overlay) unverändert wenn popupMode nicht gesetzt. |
| **FEAT-INSP** | ✅ Channel Inspector vom Mixer entkoppelt | medium (UX, user-requested) | Umgesetzt (post-v1.34.0). User-Request: "wenn mixer abgepinnt wird verschwindet der inspector anstatt mit beim mixer zu bleiben". Refactor: `ChannelInspector` aus `MixerView.tsx` extrahiert in eigene Datei `client/src/components/Mixer/ChannelInspector.tsx` mit identischer API (`part`, `parts`, `mixer`, optional `className`). `MixerView` rendert den Inspector nicht mehr intern; App.tsx mixer-tab rendered `MixerView` (oder Reattach-Stub) + `ChannelInspector` als Geschwister-Komponenten. Inspector bleibt also IMMER sichtbar in main — auch wenn der Mixer als separates Fenster abgepinnt ist. Eigene Pin-Funktion für den Inspector ist eigene Welle (kommt mit ARCH-MOD). |
...
| **Multi-Window Dockable Workspace — Mixer-Window** | ✅ Umgesetzt (post-v1.26.0). Singleton-Mixer-Popup pro Session — Pin-Button im `MixerView` Header öffnet via `createMixerWindow()` ein eigenes frameless BrowserWindow. Geteilte `DetachableWindowHeader`-Komponente (extrahiert aus PerformancePopupApp + FxPopupApp) für konsistente Pin/Close/Drag-UX. Phase-1 Scope: Channel-Strips mit Volume + Pan + Mute + Solo (shiftKey für additive) + Select, Master-Volume, BPM-Anzeige. Nicht im Popup: VU-Meter, Spectrum, FX-Inspector (bleibt im FX-Popup), Audio-Track-Strips, Export-Panel. 6 neue IPC-Channels — alle narrow-data-only. URL-Routing `?mixerPopup=1`. 14 neue Vitest-Tests. | ✅ erledigt (post-v1.26.0) |
| **Multi-Window Dockable Workspace — Sample-Browser-Window** | ✅ Umgesetzt (post-v1.27.0). Singleton-Sample-Browser-Popup. Browse-only View (Maschine-Browser-Style) mit Sample-Liste + Suche + 12 Kategorie-Filter. Click-to-Assign-Pattern: User klickt Sample im Popup → wird auf den aktuell aktiven Kanal im Haupt-Fenster gelegt. Drag-Drop zwischen Electron-Windows ist mit HTML5-DnD nicht möglich, daher Click-to-Assign analog Native Instruments Maschine. Active-Channel-Anzeige im Popup-Header. Import/Delete/Reorder bleiben im Haupt-Fenster. 6 narrow-data-only IPC-Channels. URL-Routing `?sampleBrowserPopup=1`. 17 neue Vitest-Tests. | ✅ erledigt (post-v1.27.0) |
| **Multi-Window Dockable Workspace — Pattern-Generator-Window** | ✅ Umgesetzt (post-v1.27.0). Singleton Pattern-Generator-Popup. `PatternGeneratorPopupApp` wrapped die existierende `PatternGeneratorPanel`-Komponente unverändert mit dem `DetachableWindowHeader`. Apply-Flow: PatternGeneratorPanel dispatcht den `pattern-generator:apply`-CustomEvent im Popup-Window, wir fangen ihn ab und forwarden via IPC zum Main, dort wird der Event re-dispatcht damit der existing `handleApply`-Handler in App.tsx unverändert läuft. Genre/Komplexität/Prompt-Drafts laufen über localStorage (gleicher Origin → automatisch zwischen Main + Popup geteilt). AI-API-Calls (Anthropic/OpenAI) laufen direkt im Popup-Renderer. 5 narrow-data-only IPC-Channels (open/close/is-open, set/is-always-on-top, sync:action, window:closed). URL-Routing `?patternGenPopup=1`. 15 neue Vitest-Tests. | ✅ erledigt (post-v1.27.0) |
| **Multi-Window Dockable Workspace — Window-Layout-Persistenz** | ✅ Umgesetzt (post-v1.28.0). AppStore erweitert um `popupWindowLayouts`-Schema (Singleton-Keys: performance/mixer/sampleBrowser/patternGen + per-channelId FX-Map). Pro Popup wird beim Open ein `isOpen=true`-Snapshot + (auf Wunsch) Bounds + alwaysOnTop persistiert, beim `close`-Event Bounds + alwaysOnTop überschrieben. `isAppQuitting`-Flag (gesetzt im `before-quit`-Handler) unterscheidet User-explicit-Close (→ `isOpen=false`, kein Auto-Reopen) von App-quit-cascade (→ `isOpen=true` bleibt → Auto-Reopen beim nächsten Start). Auto-Reopen passiert 800ms nach `did-finish-load` des Hauptfensters damit Main-Renderer-Action-Listener bereits gemountet sind. Schema-Migration-tolerant — alte Store-Dateien werden ohne Crash geladen. 12 neue Vitest-Tests in `tests/electron/store.test.ts`. | ✅ erledigt (post-v1.28.0) |
| **Multi-Window Dockable Workspace — Tools-Popups (3x)** | ✅ Umgesetzt (post-v1.28.0). Drei weitere Tools-sub-tabs sind jetzt pinnable: Keyboard Sampler, Chord Progressions, Pattern Library. Alle nutzen `createSimpleSingletonWindow`-Factory + `registerSimplePopupHandlers`-Helper in main.ts — drastisch reduzierte Code-Duplikation für neue singleton-style Popups. PopupApps sind wrap-and-go (nutzen den Original-Panel-Component unverändert), Pin-Button via `ToolPinButton`-Helper, Hide-Inline-When-Pinned via `ToolPopupReattachStub`-Helper. State-Sync: samples (KeyboardSampler), bpm (ChordProgressions), currentPattern + globalBpm + load-pattern-action (PatternLibrary). | ✅ erledigt (post-v1.28.0) |
| **Multi-Window Dockable Workspace — Generalisierung** | ⚠️ Verworfen — wird ersetzt durch **MIG (Dockview Workspace)** s.u. Die ad-hoc Popups werden zugunsten eines docking-library-basierten Workspaces (in-window tabs/splits + electron-popout) phase-out. | siehe MIG |
| **MIG — Dockview Workspace (komplette Architekturüberholung)** | User-Request post-v1.34.0: "alle elemente als eigenständige fenster bzw. programme schreiben, im hauptfenster verankerbar aber trotzdem eigenständig, modular wie bei einem browser". Plan in drei Phasen: (1) **MIG-1 Diagnostik** (post-v1.34.0): uncaughtException/unhandledRejection-Handler in main + Renderer; crash.log in userData mit append + 2 MB Rotation; logEvent für popup-close-Pfade. Erlaubt konkrete Crash-Reports statt verbaler "es crasht"-Berichte. (2) **MIG-2 Dockview Integration** (Welle nach v1.35.0): `dockview-react` als in-window-docking-Bibliothek. Neuer `WorkspaceShell.tsx` Container. Jeder Tab (Mixer, ChannelInspector, DrumMachine, Tools-Subs, etc.) wird ein "Dockview View". User kann Tabs ziehen, splitten, zu Gruppen fusionieren, floaten — alles in EINEM Fenster. (3) **MIG-3 Electron Popout** (Welle danach): Dockview's onWillDragOverlay + neue createDockviewWindow-IPC → echtes Multi-Window-Drag-Out. Ein gezogenes Tab wird zu eigenem BrowserWindow, drag-zurück = reattach. Bestehende ad-hoc Popups (perf/fx/mixer/sb/patternGen/etc.) werden komplett ersetzt durch dieses System. Eine Unified-Popout-Implementation. | 3 Wochen total |
...
| **Live Step Recording** | ✅ Umgesetzt (post-v1.30.0, Welle 2 in v1.32.0). Welle 1 (v1.31.0): `useLiveStepRecorder`-Hook in `client/src/hooks/`. MIDI-Note-In → GM-Drum-Category → matching part → Step im Overdub-Mode am `AudioEngine.currentStep`. Welle 2 (v1.32.0): (a) **Replace-Mode** — subscriben an `AudioEngine.onPosition`; beim Vorrücken werden alle Parts in dem neuen Step gecleart bevor neue MIDI-Hits eingehen können. Mode-Toggle in neuer `RecordSettingsPopover`-Komponente. (b) **Punch-In/Out Range** — nur Steps innerhalb [punchIn, punchOut] werden recorded, inklusive Wrap-Around (z.B. punchIn=14, punchOut=2). UI: 2 Step-Number-Inputs im Popover. (c) Persistent recording-mode + punch-fields in `useProjectStore`. Tests in `tests/features/live-step-recorder.test.ts`: 17 Mapping-Tests + 7 Punch-Range-Tests = 24 total. |
| **Audio-Eingang Recording** | ✅ Umgesetzt (post-v1.32.0). UI existierte bereits (`AudioInputRecorder.tsx`); diese Welle hat die UX poliert: (a) **Device-Picker** — Dropdown wenn mehrere Audio-Inputs verfügbar (USB-Mic / Line-In / etc.), nutzt `navigator.mediaDevices.enumerateDevices()` und passt MediaTrackConstraints mit `deviceId: { exact }`. (b) **Live-Duration-Timer** (mm:ss / h:mm:ss bei längeren Takes) während der Aufnahme neben VU-Meter. (c) **Pending-Sample-Dialog** nach Stop: User kann den Take umbenennen ODER verwerfen, bevor er als Sample registriert wird (vorher: Auto-Save mit Default-Namen). Enter speichert, Escape verwirft. (d) `formatRecordingDuration`-Helper pure-Funktion in `useAudioInput`. 9 neue Vitest-Tests für Duration-Format-Edge-Cases. |
...
- **TASK-129 Welle 3 + 4** — ✅ Vollständig erledigt: v2.51.0 Read-only Badge (SMP/WT/FM/GR neben Part-Name in ChannelStrip), v2.54.0 Direct-Mode-Switch (Klick auf Badge öffnet 4-Optionen-Menu; bei Wechsel auf wavetable/fm setzt applySourceTypeChange automatisch DEFAULT_SYNTH_PARAMS — bestehende synthParams werden behalten, nur mode wechselt). Tests: 10 Pure-Transform-Cases + 3 Playwright-E2E. Per-Note Polyphonie-Volumen-Konsistenz bleibt offen.
- **TASK-127 Welle 3** — ✅ Cmd+A E2E erledigt (v2.52.0): 4 Playwright-Cases decken Reorder-Mode-Select-All, Play-Mode-NoOp, Editor-offen-NoOp, Empty-Pads-NoOp ab. Welle 4 (Auto-Scroll-Container-Fallback) ist N/A — das Pad-Grid ist seit BUG-016 (post-v1.26) `aspect-square h-full grid-cols-4 grid-rows-4` → es gibt keinen scrollbaren Container mehr, in dem ein Fallback nötig wäre.
- **TASK-126 Welle 2** — ✅ Pad-Hold-Mode E2E erledigt (v2.59.0): 3 Playwright-Cases in `tests/web/macros.spec.ts` decken (a) `data-macro-trigger-kind="pad"` + `data-macro-trigger-mode="hold"` Attribute, (b) mouseDown dispatcht `macro:button:trigger` mit `triggerKind:"pad"` + `padIndex`, (c) 500ms-Hold → trigger feuert genau einmal + release feuert einmal + keine extra triggers nach release. Macros-Test-Suite jetzt 7 Cases (4 script-hold + 3 pad-hold).
- **TASK-122 Welle 2** — ✅ Vollständig erledigt: Token-Definition in v2.48.0 (`--ss-accent-tertiary` + `--ss-accent-warning` in @theme + allen 10 Themes + CustomTheme.colors-Backcompat). v2.59.0 schließt die Welle mit `tests/web/theme-tokens.spec.ts` ab: 4 Playwright-Cases verifizieren (a) alle 14 Pflicht-Tokens in allen 10 Themes nicht-leer, (b) speziell die v2.48-Tokens existieren in allen 10 Themes, (c) Accent-Tokens cross-theme distinct (mind. 5 von 10 unique-Werte pro Token — fängt Copy-Paste-Drift), (d) Theme-Wechsel ist live via data-theme-Attribute-Switch + Round-Trip. Kein Pixel-Screenshot-Compare nötig — CSS-Token-Vergleich ist deterministischer und kostet keine Baseline-Pflege.
