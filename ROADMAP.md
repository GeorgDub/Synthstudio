...
| **BUG-018** | ✅ Popup-X-Button closet die komplette App (Regression in v1.28.0) | **critical** (data loss risk) | Fixed (post-v1.28.0): defensiver dritter Layer für popup-window safety in `electron/main.ts`. Modul-Flag `mainWindowDestroyed` wird nur in `mainWindow.on('closed')` auf `true` gesetzt. Der `window-all-closed`-Handler verweigert `app.quit()` wenn das Flag `false` ist — Defense gegen Electron-Edge-Cases wo das Event aus einem Popup-Close heraus feuert obwohl mainWindow noch lebt. Bei `mainWindow.isVisible()===false` wird zusätzlich `show()+focus()` aufgerufen statt zu quitten. |
