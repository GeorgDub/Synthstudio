...
| **Multi-Window Dockable Workspace — Sample-Browser-Window** | ✅ Umgesetzt (post-v1.27.0). Singleton-Sample-Browser-Popup. Browse-only View (Maschine-Browser-Style) mit Sample-Liste + Suche + 12 Kategorie-Filter. Click-to-Assign-Pattern: User klickt Sample im Popup → wird auf den aktuell aktiven Kanal im Haupt-Fenster gelegt. Drag-Drop zwischen Electron-Windows ist mit HTML5-DnD nicht möglich, daher Click-to-Assign analog Native Instruments Maschine. Active-Channel-Anzeige im Popup-Header. Import/Delete/Reorder bleiben im Haupt-Fenster (potentiell destruktiv / braucht Main-Kontext für File-Dialoge). 6 narrow-data-only IPC-Channels. URL-Routing `?sampleBrowserPopup=1`. 17 neue Vitest-Tests. | ✅ erledigt (post-v1.27.0) |
| **Multi-Window Dockable Workspace — Generalisierung** | Verbleibend: Pattern Generator, Audio Workbench, Settings, Tools können in separate `BrowserWindow`s entkoppelt werden. Voraussetzungen: (a) generische Window-Registry, (b) 📌 Pin-Button pro Tab/Panel, (c) Window-Layout-Persistenz (welches Panel war wo, beim Restart wiederherstellen), (d) Web-Fallback via `window.open` (mit Hinweis dass weniger praktikabel), (e) Keyboard-Shortcuts zum Detach/Reattach. Mit FX-Window, Performance-Popup, Mixer-Window, Sample-Browser-Window sind jetzt vier Templates verfügbar. | 1-2 Wochen |
