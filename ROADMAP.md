...
| **Multi-Window Dockable Workspace — Generalisierung** | Verbleibend: Audio Workbench (braucht Live-Audio-Playback + Canvas-Waveform-Forwarding), Script Runner (braucht IPC-Bridge für Sandbox-Calls — komplex), Settings, Chord Progression Panel, Keyboard Sampler Panel. Voraussetzungen: (a) Window-Layout-Persistenz (welches Panel war wo, beim Restart wiederherstellen), (b) Web-Fallback via `window.open`, (c) Keyboard-Shortcuts zum Detach/Reattach. Mit FX-Window, Performance-Popup, Mixer-Window, Sample-Browser-Window, Pattern-Generator-Window sind jetzt fünf Templates verfügbar. | 1-2 Wochen |
